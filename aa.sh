...
install_translate() {
    ...
    echo -ne "${YELLOW}请输入管理后台监听端口 (ADMIN_PORT) [默认: 9080]: ${RESET}"
    read -r custom_admin_port
    [[ -z "$custom_admin_port" ]] && custom_admin_port="9080"

    echo -ne "${YELLOW}请输入代理服务监听端口 (PROXY_PORT) [默认: 8080]: ${RESET}"
    read -r custom_proxy_port
    [[ -z "$custom_proxy_port" ]] && custom_proxy_port="8080"
...
EOF
    ...
    echo -e "${GREEN}====================================================${RESET}"
    echo -e "${YELLOW}后台管理地址 : http://${DETECT_IP}:${custom_admin_port}/${RANDOM_ADMIN_PATH}${RESET}"
    echo -e "${YELLOW}初始化 Token : ${SETUP_TOKEN_VAL}${RESET}"
    echo -e "${YELLOW}代理服务端口 : ${custom_proxy_port}${RESET}"
