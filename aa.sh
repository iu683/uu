...
status_check() {
    ...
    if [[ -n "$dns_v6" ]]; then
        ...
        if [[ "$test_v6" == "ok" ]]; then
            ...
            echo -e "解析: ${RED}IPv6 失败 (尝试修复中...)${RESET}"
            # 补入 Google/Cloudflare 的 IPv6 DNS
            echo -e "nameserver 2001:4860:4860::8888\nnameserver 2606:4700:4700::1111" >> /etc/resolv.conf
        fi
    else
        echo -e "DNSv6: ${RED}无配置${RESET}"
        # 如果服务器本身有 IPv6 地址，但没配置 DNS，则自动补充
        if [[ -n $(ip -6 addr show | grep "inet6" | grep -v "fe80") ]]; then
            echo -e "${YELLOW}检测到系统支持 IPv6，正在自动补充 DNSv6...${RESET}"
            echo -e "nameserver 2001:4860:4860::8888\nnameserver 2606:4700:4700::1111" >> /etc/resolv.conf
        fi
    fi
