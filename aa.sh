...
fetch_latest_version() {
    ...
    # 优先通过官方 API 请求，如果失败则通过代理池轮询 API 节点
    TMP_API="$(mktemp)"
    local success=false
    
    # 第一步：尝试直接官方 API 请求
    if curl -sSL -H "Accept: application/vnd.github+json" --max-time 8 "https://api.github.com/repos/${REPO}/releases/latest" > "$TMP_API"; then
        VERSION="$(sed -n 's/.*"tag_name": *"\([^"]*\)".*/\1/p' "$TMP_API" | head -n 1)"
        if [ -n "$VERSION" ]; then success=true; fi
    fi
    
    # 第二步：若失败，通过代理池流解析发布页
    if [ "$success" = false ]; then
        warn "直连 GitHub API 获取版本失败，正在通过代理源池轮询检索..."
        for proxy in "${GITHUB_PROXY[@]}"; do
            local proxy_desc="${proxy:-官方直连}"
            info "正在尝试通过代理源 [ ${proxy_desc} ] 获取版本..."
            
            # 部分反代不支持 API 代理，因此直接拉取 Releases 网页 HTML 解析
            VERSION=$(curl -sS --max-time 8 "${proxy}https://github.com/${REPO}/releases/latest" 2>/dev/null | grep -o 'tag/[vV]*[0-9.]*' | awk -F '/' 'NR==1 {print $2}')
    ...
    if [ -z "$VERSION" ]; then
        die "无法获取最新版本号，所有镜像源尝试完毕，请检查网络连通性。"
    fi
    export VERSION
}
