...
init_config(){
    ...
    read -p "选择备份文件格式 (1: tar.gz, 2: zip，默认 tar.gz): " f
    case $f in 2) ARCHIVE_FMT="zip";; *) ARCHIVE_FMT="tar.gz";; esac
    read -p "设置备份文件保留天数（默认 $KEEP_DAYS 天）: " kd
    KEEP_DAYS=${kd:-$KEEP_DAYS}
    save_config
...
backup_now(){
    ...
    for dir in "${BACKUP_LIST[@]}"; do
        ...
        if [ "$ARCHIVE_FMT" == "tar.gz" ]; then
            tar -czf "$backup_name.tar.gz" -C "$(dirname "$dir")" "$basename"
        else
            zip -r "$backup_name.zip" "$dir" >/dev/null
        fi
    done
    ...
    cp "$BACKUP_DIR"/* "$TMP/repo/" 2>/dev/null || true
