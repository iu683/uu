...
show_vps_info() {
    # 实际Swap大小
    local swap_display
    if swapon --show=SIZE --bytes | grep -q '^/'; then
        swap_display=$(swapon --show=SIZE --bytes | awk 'NR==1{printf "%.0fMB",$1/1024/1024}')
    else
        swap_display="${SWAP_SIZE_MB:-0}MB"
    fi
    ...
    local bbr_display="${BBR_MODE:-optimized}"
    local dns_display="${PRIMARY_DNS_V4:-8.8.8.8}, ${SECONDARY_DNS_V4:-1.1.1.1}"

    echo -e "${CYAN}==================== VPS信息 ====================${NC}"
    echo -e "主机名: ${hostname_display}"
    echo -e "时区: ${TIMEZONE:-Asia/Shanghai}"
    echo -e "Swap: ${swap_display}"
    echo -e "BBR: ${bbr_display}"
    echo -e "DNS: ${dns_display}"
    echo -e "Fail2ban: ${fail2ban_display}"
    echo -e "SSH端口: ${ssh_display}"
    echo -e "${CYAN}==================================================${NC}\n"
}
