...
test_binary() {
  info "检查配置是否可启动..."
  if ! "$SNELL_BIN" -c "$SNELL_CONF" >/tmp/snell-test.log 2>&1 &
  then
    err "Snell 启动测试失败"
...
show_result() {
  ...
  echo '------------------------'
  echo '检查状态命令:'
  echo '  rc-service snell status'
  echo '  ss -tlnp | grep snell'
}
