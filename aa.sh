...
get_status_info() {
    ...
    if [ "$(docker ps -aq -f name=^/${CONTAINER_NAME}$)" ]; then
        ...
        if [[ "$net_mode" == "host" ]]; then
            ...
            # Bridge 模式：从端口映射获取端口（容器内固定为 8866）
            webui_port=$(docker inspect -f '{{(index (index .NetworkSettings.Ports "8866/tcp") 0).HostPort}}' "$CONTAINER_NAME" 2>/dev/null)
            [[ -z "$webui_port" ]] && webui_port="8866"
        fi
    else
        # 容器未安装/未部署时的返回值
...
install_playground() {
    ...
    if [[ "$volume_choice" == "2" ]]; then
        ...
        volume_declaration="" # 本地路径映射不需要在底部声明 volumes 
    else
...
show_info() {
    ...
    echo -e "${CYAN}默认登录用户名: admin${RESET}"
    echo -e "${CYAN}默认登录密码  : admin${RESET}"
    echo -e "${GREEN}========================================${RESET}"
}
