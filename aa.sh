...
get_banned_ip_count() {
    ...
    # 仅过滤出针对特定单体 IP/网段的 DROP/REJECT，排除全网（0.0.0.0/0 且不带 dpt 端口限制的才算纯 IP 封禁）
...
view_visual_rules() {
    ...
    # 2. 提取已放行的 TCP/UDP 端口（合并 v4 和 v6，去重并排序）
    ports_tcp=$( (iptables -L INPUT -n 2>/dev/null; ip6tables -L INPUT -n 2>/dev/null) | grep "ACCEPT" | grep "tcp dpt:" | sed -E 's/.*dpt:([0-9:]+).*/\1/' | tr ' ' '\n' | sort -nu | tr '\n' ' ')
    ports_udp=$( (iptables -L INPUT -n 2>/dev/null; ip6tables -L INPUT -n 2>/dev/null) | grep "ACCEPT" | grep "udp dpt:" | sed -E 's/.*dpt:([0-9:]+).*/\1/' | tr ' ' '\n' | sort -nu | tr '\n' ' ')
    ...
    # 4. 提取真正的 IP 白名单（排除所有包含端口、状态匹配、全网放行的规则）
    echo -e " ⚪ ${BLUE}IP 白名单规则 (放行特定源 IP)：${RESET}"
    local whitelist=$(iptables -L INPUT -n | grep "ACCEPT" | grep -vE "dpt:|spt:|0.0.0.0/0|state|ctstate" | awk '{print $4}' | grep -v "0.0.0.0" || true)
    local whitelist6=$(ip6tables -L INPUT -n | grep "ACCEPT" | grep -vE "dpt:|spt:|::/0|state|ctstate" | awk '{print $4}' | grep -v "::" || true)
    ...
    # 5. 提取真正的 IP 黑名单（排除针对特定端口的 DROP，只留纯 IP 封禁）
    echo -e "\n ⚫ ${RED}IP 黑名单规则 (封禁恶意源 IP)：${RESET}"
    local blacklist=$(iptables -L INPUT -n | grep -E "DROP|REJECT" | grep -vE "dpt:|spt:|0.0.0.0/0" | awk '{print $4}' | grep -v "0.0.0.0" || true)
    local blacklist6=$(ip6tables -L INPUT -n | grep -E "DROP|REJECT" | grep -vE "dpt:|spt:|::/0" | awk '{print $4}' | grep -v "::" || true)
    
    if [[ -n "$blacklist" || -n "$blacklist6" ]]; then
        for ip in $blacklist; do echo -e "    ❌ [IPv4] -> $ip"; done
        for ip in $blacklist6; do echo -e "    ❌ [IPv6] -> $ip"; done
    else
        echo -e "    (current 无特定 IP 黑名单规则)"
    fi
...
menu() {
    while true; do
        ...
        echo -e "${GREEN} 12. 彻底卸载防火墙功能${RESET}"
        ...
        case $choice in
            ...
            2)
                ...
                # 【关键安全机制】：防止用户误操作关闭自己的 SSH 端口导致失联
                if [ "$PORT" -eq "$PORT_SHOW" ]; then
                    echo -e "${RED}⚠️ 拒绝操作：端口 $PORT 是你当前的 SSH 端口！关闭它会导致你立刻断开连接并再也无法登录！${RESET}"
                    read -p "按回车返回菜单..."
                    continue
                fi
