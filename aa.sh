...
# 核心变量
CONTAINER_NAME="emby-tg-bot"  # 需对应 docker-compose.yml 中的 container_name
...
get_status_info() {
    ...
    if [ "$(docker ps -aq -f name=^/${CONTAINER_NAME}$)" ]; then
        ...
        [[ -z "$img_version" ]] && img_version="本地构建 (Latest)"

        # 从容器状态提取 WebUI 端口（内部监听的是 APP_PORT，默认 18080）
...
install_utils() {
    ...
    if [ ! -d "$BASE_DIR" ]; then
        ...
        echo -e "${YELLOW}目录 $BASE_DIR 已存在，跳过克隆，准备更新配置...${RESET}"
    fi
    ...
    echo -ne "${YELLOW}2. Emby 内部 API 地址 [默认: http://172.17.0.1:8096]: ${RESET}"
    read -r emby_base_url
    [[ -z "$emby_base_url" ]] && emby_base_url="http://172.17.0.1:8096"

    echo -ne "${YELLOW}3. Emby API 密钥 (必填): ${RESET}"
    read -r emby_api_key

    echo -ne "${YELLOW}4. 用户连接的 Emby 公网外网地址 [默认: http://${DETECT_IP}:8096]: ${RESET}"
    read -r emby_public_url
    [[ -z "$emby_public_url" ]] && emby_public_url="http://${DETECT_IP}:8096"

    echo -ne "${YELLOW}5. TG 管理员机器人 Token (必填): ${RESET}"
    read -r admin_bot_token

    echo -ne "${YELLOW}6. TG 管理员账户数字 ID (必填): ${RESET}"
    read -r admin_chat_ids

    echo -ne "${YELLOW}7. TG 客户服务机器人 Token (必填): ${RESET}"
    read -r client_bot_token

    echo -ne "${YELLOW}8. 管理员 TG 用户名 (如 @my_tg_name): ${RESET}"
    read -r admin_username

    # 自动生成随机 32 位 Master Key
...
APP_ENV=production
APP_PORT=18080
APP_BASE_URL=http://${DETECT_IP}:18080
...
EOF
    ...
    # 3. 检查并修正 docker-compose.yml 确保 container_name 一致
    # 防止官方文件没有指定 container_name 导致面板无法精准识别
    if [ -f "$COMPOSE_FILE" ]; then
        if ! grep -q "container_name:" "$COMPOSE_FILE"; then
            sed -i "/services:/a\  emby_tg_bot:\n    container_name: ${CONTAINER_NAME}" "$COMPOSE_FILE" 2>/dev/null
        fi
    fi

    echo -e "${YELLOW}正在编译并启动 EmbyTG 机器人 (需下载依赖，请耐心等待)...${RESET}"
    cd "$BASE_DIR" && docker compose up -d --build --force-recreate

    echo -e "${YELLOW}等待容器初始化 (约 5 秒)...${RESET}"
    sleep 5
...
update_utils() {
    ...
    echo -e "${YELLOW}正在从 Git 拉取最新代码并重新编译镜像...${RESET}"
    cd "$BASE_DIR" && git pull && docker compose up -d --build --remove-orphans
    echo -e "${GREEN}更新并重新编译完成！容器已处于最新状态。${RESET}"
}
