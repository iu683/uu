#!/bin/bash
# =================================================================
# Suwayomi (Tachidesk) Docker Compose 管理面板 (支持自定义挂载)
# =================================================================
...
install_utils() {
    ...
    # 支持自定义本地挂载路径
    echo -ne "${YELLOW}请输入数据目录本地挂载真实路径 [默认: ./data]: ${RESET}"
    read -r path_data
    [[ -z "$path_data" ]] && path_data="./data"

    # 预创建目录（如果是相对路径如 ./data 则在 $BASE_DIR 下创建，否则直接创建绝对路径）
    if [[ "$path_data" == "./"* ]]; then
        mkdir -p "$BASE_DIR/${path_data#./}"
    else
        mkdir -p "$path_data"
    fi
    ...
    # 动态生成符合官方标准的 docker-compose.yml 配置文件
...
services:
  suwayomi:
    ...
    environment:
      - TZ=Asia/Shanghai
    volumes:
      - ${path_data}:/home/suwayomi/.local/share/Tachidesk
...
EOF
    ...
    echo -e "${YELLOW}本地数据路径   : ${path_data}${RESET}"
...
uninstall_utils() {
    ...
    if [ "$confirm" = "y" ] || [ "$confirm" = "Y" ]; then
        if [ -f "$COMPOSE_FILE" ]; then
            ...
            echo -ne "${YELLOW}是否同时彻底删除本管理脚本主配置目录 (不会主动删除非 /opt 下的独立外部路径)？(y/n): ${RESET}"
            read -r clean_data
            if [ "$clean_data" = "y" ] || [ "$clean_data" = "Y" ]; then
                rm -rf "$BASE_DIR"
                echo -e "${GREEN}脚本主配置目录已彻底清理。${RESET}"
            fi
        else
...
menu() {
    ...
    echo -e "${GREEN}================================${RESET}"
    echo -e "${GREEN}1. 部署启动 (支持自定义路径)${RESET}"
