#!/bin/bash
# ========================================
# PostgreSQL 容器管理面板 (Docker Compose) - 最终全能完美版
# ========================================
...
function menu() {
    ...
    echo -e "${GREEN} 5. 启动 PostgreSQL${RESET}"
    echo -e "${GREEN} 6. 停止 PostgreSQL${RESET}"
    echo -e "${GREEN} 7. 重启 PostgreSQL${RESET}"
    ...
    echo -e "${GREEN}13. 修改用户密码 (含postgres)${RESET}"
    echo -e "${GREEN}14. 删除已有用户${RESET}"
    echo -e "${GREEN}15. 备份数据库 (pg_dump)${RESET}"
    echo -e "${GREEN}16. 恢复数据库 (psql 导入)${RESET}"
...
function install_app() {
    if [ -f "$CONFIG_FILE" ]; then
        echo -e "${RED}⚠️ 检测到已经安装过 PostgreSQL。${RESET}"
        pause; menu
    fi
...
function show_info() {
    ...
    docker exec -i -e PGPASSWORD="$POSTGRES_PASSWORD" postgres psql -U postgres -c "SELECT datname as \"Database\", pg_encoding_to_char(encoding) as \"Encoding\" FROM pg_database WHERE datistemplate = false AND datname NOT IN ('postgres', 'pg_catalog', 'information_schema');"
...
function create_database() {
    ...
    read -p "请输入编码(默认 UTF8): " encoding
    encoding=${encoding:-UTF8}
    
    docker exec -i -e PGPASSWORD="$POSTGRES_PASSWORD" postgres psql -U postgres -c "CREATE DATABASE \"$new_db\" WITH ENCODING '$encoding';" &>/dev/null
...
function delete_database() {
    ...
    if [[ "$confirm" == "y" || "$confirm" == "Y" || "$confirm" == "yes" ]]; then
        ...
        if [ $? -eq 0 ]; then
            ...
            echo -e "${RED}❌ 删除失败，请确认该库是否存在或当前是否有连接冲突。${RESET}"
        fi
    else
...
EOF
    echo -e "${YELLOW}✅ 用户 $new_user 创建成功并授权数据库 $grant_db 。密码: $new_pass${RESET}"
    pause; menu
}
...
GRANT ALL PRIVILEGES ON SCHEMA public TO "$new_user";
EOF
    echo -e "${YELLOW}✅ 联动创建成功。用户: $new_user 密码: $new_pass 库名: $new_db${RESET}"
    pause; menu
}
...
function change_password() {
    ...
    if [ $? -eq 0 ]; then
        ...
        if [ "$target_user" = "postgres" ]; then
            echo -e "${YELLOW}检测到修改了超级角色 postgres 密码，正在同步本地配置文件...${RESET}"
...
function delete_user() {
    ...
    if [ "$del_user" = "postgres" ]; then echo -e "${RED}❌ 安全限制：拒绝删除 postgres 超级管理员！${RESET}"; pause; menu; fi

    read -p "⚠️ 确定要彻底删除用户 '$del_user' 吗？(y/N): " confirm
    if [[ "$confirm" == "y" || "$confirm" == "Y" || "$confirm" == "yes" ]]; then
        # REASSIGN OWNED 规避由于用户拥有对象导致无法删除的报错
        docker exec -i -e PGPASSWORD="$POSTGRES_PASSWORD" postgres psql -U postgres <<EOF
...
function backup_db() {
    ...
    if [ $? -eq 0 ] && [ -s "$BACKUP_FILE" ]; then
        ...
        echo -e "${RED}❌ 备份失败，请核对库名或检查存储权限。${RESET}"
        rm -f "$BACKUP_FILE"
    fi
    pause; menu
}
