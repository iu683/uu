...
# =========================================================
# Xray VLESS + ML-KEM-768 + REALITY + Vision 管理脚本
# =========================================================
...
vless://${uuid}@${display_ip}:${port}?flow=xtls-rprx-vision&kem=ml-kem-768&encryption=none&type=tcp&security=reality&sni=${domain}&fp=chrome&pbk=${public_key}&sid=${shortid}&spx=%2F#${hostname}-PQC-Vision
EOF
}
...
show_current_config() {
    ...
    echo -e "${YELLOW}流控模式    : xtls-rprx-vision (ML-KEM-768)${RESET}"
...
show_menu() {
    ...
    echo -e "${GREEN}================================${RESET}"
    echo -e "${GREEN}     VLESS 后量子安全管理面板     ${RESET}"
    echo -e "${GREEN}================================${RESET}"
    echo -e "${GREEN}状态   :${RESET} $status"
    echo -e "${GREEN}版本   :${RESET} ${YELLOW}${version}${RESET}"
    echo -e "${GREEN}端口   :${RESET} ${YELLOW}${port_show}${RESET}"
    echo -e "${GREEN}流控   :${RESET} ${CYAN}Vision + ML-KEM-768 + REALITY${RESET}"
    echo -e "${GREEN}================================${RESET}"
    echo -e "${GREEN} 1. 安装节点 (Vision+MLKEM)${RESET}"
    echo -e "${GREEN} 2. 更新 Xray 内核${RESET}"
    echo -e "${GREEN} 3. 卸载节点及残留${RESET}"
    echo -e "${GREEN} 4. 修改现有配置${RESET}"
    echo -e "${GREEN} 5. 启动服务${RESET}"
    echo -e "${GREEN} 6. 停止服务${RESET}"
    echo -e "${GREEN} 7. 重启服务${RESET}"
    echo -e "${GREEN} 8. 查看服务日志${RESET}"
    echo -e "${GREEN} 9. 查看当前配置与链接${RESET}"
    echo -e "${GREEN}10. 配置 Socks5 出口代理${RESET}"
    echo -e "${GREEN}11. 阻断延迟优化 (SNI优选)✨${RESET}"
