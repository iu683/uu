...
# ================== 辅助生成器与校验 (去符号核心修改) ==================
# 完美解决 Shadowsocks 2022 必须 32 字节且外观无符号的需求
random_key() {
    local raw_hex
    raw_hex=$(openssl rand -hex "$KEY_BYTES" 2>/dev/null || echo "")
    if [[ -n "$raw_hex" ]]; then
        # 截取前32位字母数字，通过 base64 转换后，将其中的 + / = 替换成字母
        echo -n "$raw_hex" | head -c "$KEY_BYTES" | base64 | tr -d '\n' | tr '+/=' 'abc'
    else
        echo -n "PurePasswordStandard32BytesKeySSs"
    fi
}
