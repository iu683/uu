#!/usr/bin/env bash
#
# sing-box (VLESS+WS+TLS) 核心控制面板 [Alpine Linux 专属完美权限版]
...
# 强行赋予自定义证书多级父目录检索穿透及白名单读取权限的核心函数
...
inst_cert() {
  ...
  echo -e " 1) Acme自动申请(需要开放80端口)${YELLOW}（默认）${RESET}"
  echo -e " 2) 自定义证书路径"
  echo "---------------------------------------------"
  local certInput
  ...
  if [[ $certInput == 1 ]]; then
    ...
    if [[ ! -f "$acme_cmd" ]]; then
      # 100% 兼容 Alpine 开箱即用的随机合规谷歌邮箱前缀生成逻辑
      local rand_prefix
      rand_prefix=$(openssl rand -hex 6 2>/dev/null || date +%s | tail -c 8)
      curl https://get.acme.sh | sh -s email="${rand_prefix}@gmail.com"
    fi
    ...
    # 核心改动：在 --install-cert 的命令里追加 --reloadcmd 挂载面板的重启机制，确保持续续签时自动重启
    # 同时在初次安装成功分支里，显式触发一次重启同步
