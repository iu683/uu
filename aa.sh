...
uninstall_xray() {
  local choice
  reading "确定要彻底从本服务器中移除 Xray-Argo 网络核心组件吗? (y/n): " choice
  case "${choice}" in
    y|Y)
      yellow "执行整体服务集群停开与反安装..."
      if is_alpine; then
...
view_logs() {
  ...
  green "================================="
  local log_choice
  reading "请输入业务需求 [0-3]: " log_choice
...
manage_argo_menu() {
  ...
  if [[ "$argo_exist" -eq 2 ]]; then
    yellow "系统检测到 Argo 回路未安装，拒绝调度子面板！"
    sleep 15; return
  fi
  ...
  case "${ca}" in
    ...
    3)
      ...
      reading "请输入你的绑定的独立公网二级域名: " argo_domain
...
menu() {
  while true; do
    ...
    echo -e "${GREEN}=================================${RE}"
    echo -e " ${GREEN}1. 自动化一键纯净安装部署${RE}"
    echo -e " ${GREEN}2. Xray 服务状态子管控${RE}"
    echo -e " ${GREEN}3. Argo 隧道网关子管控${RE}"
