...
EOF
  ...
  # 【修复核心3】剥离双轨 IP 逻辑，无缝支持 IPv6 节点的客户端协议转换
...
V2rayN / AnXray 配置分享链接:
hysteria2://$auth_pwd@$last_ip:$port?insecure=1&sni=$hy_domain#$HOSTNAME-hy2

Surge 4+ 配置格式:
$HOSTNAME-hy2 = hysteria2, $url_ip, $port, password=$auth_pwd, skip-cert-verify=true, sni=$hy_domain
EOF
