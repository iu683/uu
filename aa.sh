...
if systemctl is-active --quiet cloudflared.service; then
    argo_status="${GREEN}运行中${RESET}"
else
    argo_status="${RED}已停止${RESET}"
fi

if systemctl is-active --quiet xray.service; then
    xray_status="${GREEN}运行中${RESET}"
else
    xray_status="${RED}已停止${RESET}"
fi


echo -e "${GREEN}"=========服务管理控制台========${RESET}"
echo
echo -e "${YELLOW}ARGO 状态 : $argo_status${RESET}"
echo -e "${YELLOW}XRAY 状态 : $xray_status${RESET}"
echo
echo -e "${GREEN} 1.管理 TUNNE$L${RESET} "
echo -e "${GREEN} 2.启动服务${RESET}"
echo -e "${GREEN} 3.停止服务${RESET}"
echo -e "${GREEN} 4.重启服务${RESET}"
echo -e "${GREEN} 5.卸载服务${RESET}"
echo -e "${GREEN} 6.查看当前链接${RESET}"
echo -e "${GREEN} 0.退出${RESET}"
echo -ne "${YELLOW}请选择菜单[默认0] : ${RESET}"
if [ -z "\$menu" ]
then
	menu=0
fi
...
show_main_menu() {
clear
echo -e "${RESET}======ARGO 管理系统==========${RESET}"
echo -e "${GREEN} 1.安装模式 (无需域名)${RESET} "
echo -e "${GREEN} 2.安装服务(需要域名)${RESET} "
echo -e "${GREEN} 3.卸载服务${RESET} "
echo -e "${GREEN} 4.清空缓存${RESET} "
echo -e "${GREEN} 5.管理服务${RESET} "
echo -e "${GREEN} 0.退出脚本${RESET} "
echo -ne "${YELLOW}请选择模式: ${RESET}"
}
