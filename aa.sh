...
get_public_ip() {
    ...
    for url in "https://api.ipify.org" "https://ip.sb" "https://checkip.amazonaws.com"; do
        ip=$(wget -4qO- --timeout=5 --tries=1 --no-check-certificate "$url" 2>/dev/null) && [[ -n "$ip" ]] && echo "$ip" && return
    done
    for url in "https://api64.ipify.org" "https://ip.sb"; do
        ip=$(wget -6qO- --timeout=5 --tries=1 --no-check-certificate "$url" 2>/dev/null) && [[ -n "$ip" ]] && echo "$ip" && return
    done
