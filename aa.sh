...
# 优先通过 ip.sb 获取公网 IP，若失败则使用 icanhazip.com 作为备用
...
# 4. 新增模块：Docker 运行状态监控
DOCKER_STATUS="🔌 未安装 Docker"
...
EOF
    ...
    # 巧妙转换 Cron 时间，过滤掉前导0（防止被 cron 误识别为八进制导致错误）
...
while true; do
    clear
    echo -e "${GREEN}===========================================${NC}"
    echo -e "${GREEN}     流量 & VPS & Docker 状态 TG通知工具    ${NC}"
    echo -e "${GREEN}===========================================${NC}"
    echo -e "${GREEN} 1. 安装${NC}"
    echo -e "${GREEN} 2. 修改配置参数${NC}"
    echo -e "${GREEN} 2. 发送测试报表${NC}"
    ...
    case $choice in
        ...
        3) 
            if [ -f "$BIN_PATH" ]; then
                /bin/bash "$BIN_PATH" && echo "✅ 测试报表已触发发送！" || echo "❌ 发送失败，请检查配置或网络。"; 
            else
