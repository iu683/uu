...
# ==============================================================================
#   Usque (MASQUE-WARP) 面板 - 经典原版菜单样式 & 纯 v6 极速全自动修复版
# ==============================================================================
...
register_usque() {
    ...
    local cp_resolv=0
    if [ "$has_v4" -ne 1 ]; then
        info "检测到当前环境为纯 IPv6 独享机，配置临时高连通 DNS64 管道..."
    ...
    [ -d "$CONF_DIR" ] || mkdir -p "$CONF_DIR"
    # 强制清理老旧无效遗留配置文件，防止冲突挂起
    rm -f "$CONF_FILE"
    cd "$CONF_DIR" || exit 1
    
    # 强制采取最高连通性的“原生匿名注册”路线，杜绝 Team Token 跨区鉴权失败
    info "正在向 Cloudflare 发起免交互全自动账户资产申请..."
    local reg_cmd=("${INSTALL_BIN}" "register")

    if "${reg_cmd[@]}"; then
        ok "Cloudflare 凭据资产注册完成。"
        
        if [ -f "$CONF_FILE" ]; then
            if [ "$has_v4" -ne 1 ]; then
                info "正在对纯 IPv6 环境进行核心配置文件 [多维端点硬核焊死] 欺骗清洗..."
                
                # 提取刚才生成的黄金私钥与设备 ID，防止丢失
                ...
                if [ -n "$p_key" ] && [ -n "$d_id" ]; then
                    # 彻底重组极简高内聚配置，消除任何非 v6 端点导致的 login failed 假死
                    cat <<EOF > "$CONF_FILE"
{
  "private_key": "${p_key}",
  "device_id": "${d_id}",
...
EOF
                    ok "清洗成功：全部出站端点已暴击重定向至 -> [2606:4700:102::1]"
                else
                    warn "配置文件中未提炼到私钥凭证，跳过重构清洗。"
                fi
            else
                ok "双栈/V4 环境验证通过，保留官方原生 endpoint 配置。"
            fi
        fi
    else
        [ "$cp_resolv" -eq 1 ] && mv -f /etc/resolv.conf.bak /etc/resolv.conf
        die "设备向云端注册资产失败，请检查机器的外部出站路由。"
    fi
    [ "$cp_resolv" -eq 1 ] && mv -f /etc/resolv.conf.bak /etc/resolv.conf
}
