...
# =========================================================
# Shadowsocks-Rust + Shadow-TLS 一体化管理脚本 (Alpine 专属版)
...
check_deps() {
    ...
    # 确保 edge 或 main 源可用并安装必备组件
    apk update
    apk add --no-cache curl wget tar xz openssl iproute2 ibm-errlog-flt 2>/dev/null || true
...
random_port() {
    # Alpine sh 不支持 shuf，用 awk 替代随机端口生成
