...
detect_os() {
    if [ -f /etc/alpine-release ]; then
        ...
        echo -e "${RED}未识别的系统，脚本可能无法完美运行。${RESET}"
        OS="unknown"
        HTTP_D="/etc/nginx/conf.d"
    fi
}
...
get_panel_status() {
    if [ "$OS" = "alpine" ]; then
        if rc-service nginx status 2>&1 | grep -q "started"; then
            STATUS="${GREEN}已启动 (OpenRC)${RESET}"
        else
            STATUS="${RED}未运行${RESET}"
        fi
    else
        if systemctl is-active --quiet nginx 2>/dev/null; then
            STATUS="${GREEN}已启动 (Systemd)${RESET}"
        else
            STATUS="${RED}未运行${RESET}"
        fi
    fi
...
install_or_init_acme_sh() {
    ...
    if ! is_valid_email "$ACME_EMAIL"; then
        echo -e "${RED}邮箱格式不合法: $ACME_EMAIL${RESET}"
        return 1
    fi
...
add_common_site() {
    ...
    prompt UP_HOST "请输入反代后端 IP/主机名 (如 127.0.0.1)" "127.0.0.1"
    prompt UP_PORT "请输入反代后端端口" "80"
...
add_custom_cert_site() {
    echo -e "${YELLOW}=== 添加配置 (使用自备/自定义证书) ===${RESET}"
    prompt DOMAIN "请输入反代域名"
    DOMAIN="$(strip_scheme "$DOMAIN")"
    [ -n "$DOMAIN" ] || return 1
    ...
    prompt UP_HOST "请输入反代后端 IP/主机名" "127.0.0.1"
    prompt UP_PORT "请输入反代后端端口" "80"
    is_port "$UP_PORT" || return 1

    prompt CUSTOM_CERT "请输入自备证书 (.crt/.cer/fullchain) 文件的绝对路径"
    prompt CUSTOM_KEY "请输入自备私钥 (.key/private.key) 文件的绝对路径"
...
# 9. Emby 专属流媒体反代配置
add_emby_site() {
    prompt DOMAIN "请输入 Emby 绑定的域名"
    DOMAIN="$(strip_scheme "$DOMAIN")"
    [ -n "$DOMAIN" ] || return 1

    prompt LISTEN_PORT "请输入 HTTPS 监听端口" "52443"
    is_port "$LISTEN_PORT" || return 1

    prompt UP_HOST "请输入 Emby 上游主机名或IP (不要带 https://)"
    UP_HOST="$(strip_scheme "$UP_HOST")"
    
    prompt UP_PORT "请输入 Emby HTTPS 端口" "443"
    is_port "$UP_PORT" || return 1
    ...
    cat > "$conf_path" <<EOF
# META domain=${DOMAIN} port=${LISTEN_PORT} upstream=https://${UP_HOST}:${UP_PORT} type=emby
server {
    listen ${LISTEN_PORT} ssl;
    listen [::]:${LISTEN_PORT} ssl;
    server_name ${DOMAIN};
...
list_sites() {
    ...
    if [ -d "$HTTP_D" ]; then
        for f in "${HTTP_D}/${CONF_PREFIX}"*.conf; do
            ...
            echo -e "◈ 域名: ${GREEN}${domain}${RESET} | 监听端口: ${YELLOW}${port}${RESET} | 后端: ${upstream}"
        done
    fi
...
test_cert_renew() {
    if [ -x "${ACME_HOME}/acme.sh" ]; then
        echo "==> 开始模拟测试证书续期 (Cron 触发测试)..."
        "${ACME_HOME}/acme.sh" --cron --home "$ACME_HOME" --dry-run
    else
        echo -e "${RED}未安装 acme.sh 证书环境${RESET}"
    fi
}
...
main_panel() {
    ...
    while true; do
        ...
        echo -e "${GREEN}  4. 修改/查看已有配置${RESET}"
        echo -e "${GREEN}  5. 删除反代配置${RESET}"
        echo -e "${GREEN}  6. 测试证书续期${RESET}"
        echo -e "${GREEN}  7. 查看证书状态${RESET}"
        echo -e "${GREEN}  8. 查看当前站点${RESET}"
        echo -e "${GREEN}  9. Emby反代配置${RESET}"
        echo -e "${GREEN} 10. 重载Nginx服务${RESET}"
        ...
        case "$choice" in
            ...
            4)  list_sites ; pause ;;
            5)  delete_site ; pause ;;
            6)  test_cert_renew ; pause ;;
            7)  check_domains_status ;;
            8)  list_sites ; pause ;;
            9)  add_emby_site ; pause ;;
            10) manage_nginx_service reload && echo -e "${GREEN}重载成功！${RESET}" ; pause ;;
            11) uninstall_nginx ; pause ;;
            0)  clear && exit 0 ;;
