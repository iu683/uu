#!/usr/bin/env bash
#
# sing-box (AnyTLS) 定制管理面板 (支持密码自定义)
...
EOF
  ...
  # 3. 封装你指定的通用高亮回显文本并固化持久化
  cat << EOF > "$SB_DIR/url.txt"
${GREEN}====== AnyTLS 节点信息 ======${RESET}
${YELLOW}IP      : ${ip}${RESET}
${YELLOW}端口    : $port${RESET}
${YELLOW}密码    : $auth_pwd${RESET}
${GREEN}---------------------------${RESET}
${YELLOW}📄 V6VPS 请自行替换 IP 地址为 V6 ★${RESET}
${YELLOW}[信息] V2rayN 链接：${RESET}
${CYAN}anytls://$auth_pwd@$url_ip:$port/?insecure=1#$hostname-Anytls${RESET}
${YELLOW}[信息] Surge 配置：${RESET}
${CYAN}$hostname-Anytls = anytls, $url_ip, $port, password=$auth_pwd, tfo=true, skip-cert-verify=true, reuse=false${RESET}
${YELLOW}---------------------------------${RESET}
EOF
...
showconf() {
  if [[ ! -f "$SB_DIR/url.txt" ]]; then
    error "未找到 AnyTLS 节点配置信息，请确保已成功部署节点。"
    return
  fi
  # 精准还原高亮节点回显
  cat "$SB_DIR/url.txt"
