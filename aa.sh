...
inst_cert() {
    ...
    while true; do
      ...
      if [[ -f "$user_cert" && -f "$user_key" ]]; then
        ...
        if fix_external_cert_permission "$user_cert" "$user_key"; then
          ...
          # 2. 【核心修复】使用 -h 参数，强制只修改软链接“快捷方式本身”的所有权，绝不穿透
  ...
  # 只有在非自定义证书（选项1、2）时，才执行全局覆盖，防止选项3的软链接穿透破坏外部文件
  if [[ $certInput -ne 3 ]]; then
    chmod 644 "$cert_path" 2>/dev/null || true
    chmod 600 "$key_path" 2>/dev/null || true
    chown -R ${RUN_USER}:${RUN_USER} "$CONFIG_DIR/certs" 2>/dev/null || true
  fi
}
