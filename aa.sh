...
# 定义所需的所有基础依赖组件
REQUIRED_CMDS="curl tar sed grep awk iptables"
MISSING_CMDS=""
...
if [ -n "$MISSING_CMDS" ]; then
    info "检测到系统缺失必要基础组件:${YELLOW}$MISSING_CMDS${RESET}，正在尝试自动修复..."
    ...
    case "$OS" in
        ...
        centos|rhel|rocky|almalinux|fedora)
            ...
            ;;
        *)
            die "未知的操作系统，请手动安装缺失组件: $MISSING_CMDS"
            ;;
    esac
    ...
    for cmd in $MISSING_CMDS; do
        if ! command -v "$cmd" &> /dev/null; then
            die "自动安装组件 [ $cmd ] 失败，请检查网络或系统源后手动运行包管理器安装。"
        fi
    done
    ok "所有基础组件已自动补全成功！"
fi
