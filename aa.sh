...
mount_remote() {
    ...
    read -p "请输入网盘内的存储桶名称 (如 sss，建议只输桶名防止I/O错误): " remote_dir
...
Type=simple
User=root
# 核心参数优化：加入了权限允许、写入缓存、24小时过期、32M内存缓冲、5分钟自动刷新
...
EOF
    ...
    if systemctl is-active --quiet "rclone-mount@${remote}"; then
        echo -e "\n${GREEN}✅ 已成功将网盘 [${mount_source}] 挂载到本地 [${path}]！${RESET}"
        echo -e "${GREEN}ℹ️ 缓存刷新时间设为 5 分钟，新文件会在 5 分钟内自动同步。${RESET}"
