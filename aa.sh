...
get_latest_snell_version() {
    ...
    latest_version=$(curl -sL -A "Mozilla/5.0 (Windows NT 10.0; Win64; x64) AppleWebKit/537.36" \
        ...
        grep -oE 'v6\.[0-9]+\.[0-9]+' | head -n 1 2>/dev/null || echo "")
...
download_and_extract_snell() {
    ...
    case "$ARCH" in
        aarch64|arm64)
            ...
            ;;
        armv7l|armhf|armv8l)
            SNELL_URL="https://dl.nssurge.com/snell/snell-server-${VERSION}-linux-armv7l.zip"
            ;;
        x86_64|amd64)
    ...
    echo -e "${GREEN}[信息] 正在下载适用于 ${ARCH} 的 Snell-${VERSION}...${RESET}"
...
install_snell() {
    ...
    # systemd 服务单元 (v6 移除了过时的 ipv6 布尔开关，改用 listen 统一控制)
...
show_menu() {
    ...
    if [ -x "$SNELL_DIR/snell-server" ]; then
        ...
        [ -z "$VERSION_SHOW" ] && VERSION_SHOW=$("$SNELL_DIR/snell-server" --version 2>&1 | grep -oE 'v[0-9]+\.[0-9]+\.[0-9]+')
