...
show_menu() {
    ...
    get_status
    echo -e "${GREEN}================================================${RESET}"
    echo -e "${GREEN}    ◈  OpenCode CLI  标准权限管理面板  ◈    ${RESET}"
    echo -e "${GREEN}================================================${RESET}"
...
# 4. 连接/添加 API 密钥 (修正为官方规定的 providers 命令)
login_opencode() {
    get_paths
    if [ -n "$REAL_EXEC_PATH" ]; then
        echo -e "\n${YELLOW}正在调用 OpenCode 凭据连接程序 (providers)...${RESET}"
        if [[ "$REAL_EXEC_PATH" == "/root/"* ]]; then
            sudo "$REAL_EXEC_PATH" providers
        else
            opencode providers
        fi
    else
