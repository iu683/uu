#!/bin/bash
# ========================================
# MariaDB 容器管理面板 (Docker Compose) - 智能交互进阶版
# ========================================
...
function menu() {
    ...
    echo -e "${GREEN}10. ${RED}删除数据库 (列出已创)${RESET}"
    echo -e "${GREEN}11. 创建用户${RESET}"
    echo -e "${GREEN}12. ${RED}删除用户 (列出已创)${RESET}"
    echo -e "${GREEN}13.${RESET} ${YELLOW}创建数据库+用户${RESET}"
    echo -e "${GREEN}14. 修改用户密码${RESET}"
    echo -e "${GREEN}--------------------------------${RESET}"
    echo -e "${GREEN}15. 备份数据库 (支持自定目录)${RESET}"
    echo -e "${GREEN}16. 恢复数据库 (支持自定目录)${RESET}"
    echo -e "${GREEN}--------------------------------${RESET}"
...
function delete_database() {
    ...
    read -p "⚠️ 危险警告：确定要彻底删除自定义库 [$del_db] 吗？数据将灰飞烟灭！(y/N): " confirm
...
function delete_user() {
    ...
    read -p "⚠️ 确定要注销业务用户 '$del_user' 吗？(y/N): " confirm
    if [[ "$confirm" == "y" || "$confirm" == "Y" ]]; then
