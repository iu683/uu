...
function install_app() {
    ...
    # 自定义 URL
    read -p "请输入 UPTIME_KUMA_BASE_URL [默认:http://127.0.0.1:3883]: " input_url
    UPTIME_KUMA_BASE_URL=${input_url:-"http://127.0.0.1:3883"}

    # 自定义端口（只影响本地映射）
    read -p "请输入 Web 端口 [默认:3883]: " input_port
    HOST_PORT=${input_port:-3883}

    # 页面 ID
    read -p "请输入 PAGE_ID [默认:test1]: " input_page
    PAGE_ID=${input_page:-test1}
    ...
    if [ ! -d "$APP_DIR/.git" ]; then
        echo -e "${GREEN}克隆 Kuma Mieru 仓库...${RESET}"
        git clone https://github.com/Alice39s/kuma-mieru.git "$APP_DIR"
    else
        echo -e "${YELLOW}源码已存在，跳过克隆${RESET}"
    fi
...
EOF
    ...
    echo -e "${YELLOW}🌐 Web 地址: $UPTIME_KUMA_BASE_URL${RESET}"
