...
if [ -n "$MISSING_CMDS" ]; then
    echo -e "${BLUE}[INFO]检测到系统缺失必要组件:${YELLOW}$MISSING_CMDS${BLUE}，正在自动安装...${RESET}"
...
fetch_latest_version() {
    echo -e "${BLUE}[INFO]正在轮询获取 Xray-core 最新 Release 版本号...${RESET}"
    VERSION=""
...
download_bin() {
    ...
    for proxy in "${GITHUB_PROXIES[@]}"; do
        ...
        echo -e "${BLUE}[INFO]正在尝试通过镜像源 [ ${CYAN}${proxy:-官方直连}${BLUE} ] 下载资产包...${RESET}"
...
menu_install() {
    ...
    echo -e "${BLUE}[INFO]正在安全重载实例配置项并拉起: ${CURRENT_INSTANCE} ...${RESET}"
...
menu_uninstall() {
    ...
    if [ -d "$CONFIG_DIR" ] && [ -z "$(ls -A "$CONFIG_DIR" | grep 'config_')" ]; then
        echo -e "${BLUE}[INFO]检测到所有 Reality 节点已排空，执行全局核心组件垃圾回收机制...${RESET}"
...
while true; do
    ...
    echo -e "${GREEN}11. 配置自定义Socks5出口${RESET} ${CYAN}← 链式分流代理${RESET}"
