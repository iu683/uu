...
ExecStart=$ANYTLS_EXECUTABLE_INSTALL_PATH -l :\${ANYTLS_PORT} -p \${ANYTLS_PASSWORD} --cert \${ANYTLS_CERT_PATH} --key \${ANYTLS_KEY_PATH}
Restart=always
RestartSec=3
...
_anytls_configure_certificate() {
  ...
  case "$certInput" in
    1)
      _anytls_info "正在生成必应 (www.bing.com) 自签名证书..."
      ...
      if openssl req -x509 -nodes -days 3650 -newkey rsa:2048 \
        ...
        _anytls_info "自签证书生成成功！"
      else
        _anytls_error "OpenSSL 证书生成失败，请确认系统已安装 openssl。"
        return 1
      fi
      ;;
    ...
    2)
      ...
      _anytls_info "正在通过 Acme 构建域名证书环境（域名: ${acme_domain}）..."
      # 此处留存与 Toolbox 内部通用 Acme 脚本对接空间
      # 默认生成空桩文件确保主程序可以通过验证
      touch "$cert_path" "$key_path" 
      _anytls_warn "注意：此处为依赖对接桩，真实生产环境请确保 Acme 签发脚本逻辑已绑定在该路径。"
      ;;
    ...
    3)
      ...
      while true; do
        ...
        if [[ -f "$cert_path" ]]; then
          ...
          _anytls_error "找不到该证书文件，请重新确认路径！"
        fi
      done
      ...
      while true; do
        ...
        if [[ -f "$key_path" ]]; then
          ...
          _anytls_error "找不到该私钥文件，请重新确认路径！"
        fi
      done
      ;;

    *)
      _anytls_error "无效输入，强制切回自签名证书。"
      cert_path="${ANYTLS_CERT_DIR}/anytls.crt"
      key_path="${ANYTLS_CERT_DIR}/anytls.key"
      openssl req -x509 -nodes -days 3650 -newkey rsa:2048 -keyout "$key_path" -out "$cert_path" -subj "/CN=www.bing.com" >/dev/null 2>&1
      ;;
  esac
}
...
EOF
  ...
  if _anytls_has_command systemctl; then
    ...
    if _anytls_systemctl is-active --quiet anytls-server 2>/dev/null; then
      _anytls_info "AnyTLS-Go 服务配置并启动成功！"
    else
      _anytls_error "AnyTLS-Go 服务启动失败，请运行 'journalctl -u anytls-server' 查看日志。"
    fi
  else
    pkill -f "$ANYTLS_EXECUTABLE_INSTALL_PATH" || true
    "$ANYTLS_EXECUTABLE_INSTALL_PATH" -l :${port} -p ${auth_pwd} --cert ${cert_path} --key ${key_path} >/dev/null 2>&1 &
    _anytls_info "非 systemd 环境，程序已挂载至后台后台 Pid 守护池。"
  fi
  anytls_showconf
}
...
anytls_update() {
  ...
  _anytls_warn "检测到新版本，即将开始平滑更新 (你的配置与证书参数不会改变)..."
  ...
  if _anytls_has_command systemctl; then
    ...
    _anytls_systemctl restart anytls-server >/dev/null 2>&1 || true
    if _anytls_systemctl is-active --quiet anytls-server 2>/dev/null; then
      _anytls_info "AnyTLS-Go 已成功平滑更新至 v${latest_version}！"
    else
      _anytls_error "核心更新成功，但服务重启失败，请检查错误日志。"
    fi
  else
    pkill -f "$ANYTLS_EXECUTABLE_INSTALL_PATH" || true
    source "$ANYTLS_CONFIG"
    "$ANYTLS_EXECUTABLE_INSTALL_PATH" -l :${ANYTLS_PORT} -p ${ANYTLS_PASSWORD} --cert ${ANYTLS_CERT_PATH} --key ${ANYTLS_KEY_PATH} >/dev/null 2>&1 &
    _anytls_info "AnyTLS-Go 核心已更新并于后台重启运行。"
  fi
}
...
anytls_changeconf() {
  ...
  local ANYTLS_PORT ANYTLS_PASSWORD ANYTLS_CERT_PATH ANYTLS_KEY_PATH
  source "$ANYTLS_CONFIG"
  ...
  read -rp "是否重新配置证书选项？[y/N]: " re_cert
  if [[ "$re_cert" =~ ^[Yy]$ ]]; then
    _anytls_configure_certificate
  fi

  _anytls_write_and_show_config
  _anytls_info "配置修改并应用成功！"
}
...
anytls_showconf() {
  ...
  echo -e "${YELLOW}证书路径  : ${ANYTLS_CERT_PATH}${RESET}"
  echo -e "${YELLOW}密钥路径  : ${ANYTLS_KEY_PATH}${RESET}"
  echo -e "${GREEN}---------------------------------------------${RESET}"
  echo -e "${YELLOW}📄 V6VPS 请自行替换 IP 地址为 V6 ★${RESET}"
  echo -e "${YELLOW}[信息] V2rayN 配置分享链接:${RESET}"
  echo -e "${CYAN}anytls://${ANYTLS_PASSWORD}@${vps_ip}:${ANYTLS_PORT}/?insecure=1#${hostname}-Anytls${RESET}"
  echo -e "${YELLOW}[信息] Surge 配置格式:${RESET}"
  echo -e "${CYAN}${hostname}-Anytls = anytls, ${vps_ip}, ${ANYTLS_PORT}, password=${ANYTLS_PASSWORD}, tfo=true, skip-cert-verify=true, reuse=false${RESET}"
  echo -e "${GREEN}=============================================${RESET}"
}
