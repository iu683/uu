...
run_test() {
    ...
    # 1. 测试大包 (1024 字节)
    echo "大包测试（1024 Bytes）："
    local raw_big=$(nexttrace --tcp --psize 1024 "$ip" -p 80 2>/dev/null)
    # 【提纯核心】: 剔除颜色代码 -> 只要第一列是纯数字的行 -> 打印跳数和IP
    route_big=$(echo "$raw_big" | sed 's/\x1b\[[0-9;]*m//g' | awk '$1 ~ /^[0-9]+$/ {print $1, $2}')

    # 2. 测试小包 (64 字节)
    echo "小包测试（64 Bytes）："
    local raw_small=$(nexttrace --tcp --psize 64 "$ip" -p 80 2>/dev/null)
    route_small=$(echo "$raw_small" | sed 's/\x1b\[[0-9;]*m//g' | awk '$1 ~ /^[0-9]+$/ {print $1, $2}')

    echo -e "\n${YELLOW}=== 路由对比 ===${RESET}"
    
    # 3. 结果严格判定
    if [ -z "$route_big" ] || [ -z "$route_small" ]; then
        echo -e "${RED}❌ 测试超时或未获取到有效路由数据。${RESET}"
        echo -e "${YELLOW}💡 提示: 请确认当前节点与靶点 IP (${ip}) 网络是否本身可达。${RESET}"
    else
        diff_output=$(diff <(echo "$route_big") <(echo "$route_small") 2>/dev/null)
        if [ -z "$diff_output" ]; then
            echo -e "${GREEN}大小包路由一致 ✅${RESET}"
        else
            echo -e "${RED}大小包路由不一致 ❌ (成功抓到商家策略分流欺骗！)${RESET}"
            echo -e "${BLUE}>>> 轨迹差异对比 ( - 大包 / + 小包 ) <<<${RESET}"
            echo "$diff_output"
        fi
    fi
