...
menu_install() {
    ...
    read -p "$(echo -p "${GREEN}请输入监听 IP 地址 [默认: 127.0.0.1]: ${RESET}")" input_ip
    local opt_ip="${input_ip:-127.0.0.1}"

    read -p "$(echo -p "${GREEN}请输入 SOCKS5 监听端口 [默认: 1080]: ${RESET}")" input_port
    ...
    if [ "$opt_ip" != "127.0.0.1" ] && [ "$opt_ip" != "localhost" ]; then
        ...
        while true; do
            read -p "$(echo -e "${GREEN}请输入鉴权用户名 (不能为空): ${RESET}")" opt_user
            [ -n "$opt_user" ] && break
            echo -e "${RED}[错误] 用户名不能为空，请重新输入。${RESET}"
        done
        while true; do
            read -p "$(echo -e "${GREEN}请输入鉴权密码 (为了安全，内核要求必须 ≥16 位): ${RESET}")" opt_pass
            if [ ${#opt_pass} -ge 16 ]; then break; fi
            echo -e "${RED}[安全终止] 密码长度不够。公网暴露必须使用 16 位及以上的强密码！${RESET}"
        done
    else
        read -p "$(echo -e "${GREEN}请输入鉴权用户名 (本地回环默认留空不启用): ${RESET}")" opt_user
        if [ -n "$opt_user" ]; then
...
menu_edit_config() {
    ...
    read -p "$(echo -e "${GREEN}请输入监听 IP 地址 [当前: ${current_ip}]: ${RESET}")" input_ip
    local opt_ip="${input_ip:-$current_ip}"

    read -p "$(echo -e "${GREEN}请输入 SOCKS5 监听端口 [当前: ${current_port}]: ${RESET}")" input_port
    ...
    if [ "$opt_ip" != "127.0.0.1" ] && [ "$opt_ip" != "localhost" ]; then
        ...
        while true; do
            read -p "$(echo -e "${GREEN}${prompt_user_desc}: ${RESET}")" input_user
        ...
        while true; do
            read -p "$(echo -e "${GREEN}${prompt_pass_desc}: ${RESET}")" input_pass
        ...
        read -p "$(echo -e "${GREEN}${p_user_text}: ${RESET}")" input_user
        ...
        if [ -z "$input_user" ] && [ -n "$current_user" ]; then
            ...
            opt_user="$input_user"
            if [ -n "$opt_user" ]; then
                read -p "$(echo -e "${GREEN}请输入鉴权密码: ${RESET}")" input_pass
                opt_pass="$input_pass"
            fi
        fi
    fi
...
setup_transparent_proxy() {
    ...
    echo -e "${YELLOW}配置 Google 流量透明代理...${RESET}"
...
exit 101' > /usr/sbin/policy-rc.d
    ...
    info "正在使用包管理器部署必要底层组件 (redsocks / iptables)..."
    case $OS in
        ubuntu|debian)
            apt-get update -y -qq >/dev/null 2>&1
            apt-get install -y -qq redsocks iptables >/dev/null 2>&1
            ;;
        centos|rhel|rocky|almalinux|fedora)
            ...
            ;;
        *)
            apt-get install -y redsocks iptables 2>/dev/null || yum install -y redsocks iptables 2>/dev/null
            ;;
    esac
...
status() {
    ...
    # 核心点：直接使用原生的 curl 去请求 8.8.8.8。
    # 如果 iptables 透明代理生效了，这个本来属于 Google IP 的 8.8.8.8 会被强行拦截并塞入 redsocks 缓存，
    # 进而被塞进 warp-rust SOCKS5。
    # 只要我们在网头中添加 Host: 1.1.1.1，Cloudflare 的边缘节点就会吐出 /cdn-cgi/trace。
    # 反之，如果代理没生效，直接向 8.8.8.8 发这个请求，Google 无法识别这个 Host，会返回 HTTP 404 或无响应！
...
EOF
    ...
    echo -e "${GREEN} Google 流量现已自动走WARP出口 ${RESET}"
    echo -e "${GREEN}==============================${RESET}"
    echo ""
}
...
while true; do
    ...
    echo -e "${GREEN}分流 :${RESET} ${YELLOW}${panel_port}${RESET}"
    echo -e "${GREEN}==============================${RESET}"
    ...
    echo -e "${GREEN}──────────────────────────${RESET}"
    echo -e "${GREEN}10. 开启谷歌分流WARP${RESET}"
    echo -e "${GREEN}11. 关闭谷歌分流WARP${RESET}"
    echo -e "${GREEN}12. 查看分流状态${RESET}"
