...
install_deps() {
  info "安装依赖..."
  apk update
  apk add bash wget curl unzip iproute2 coreutils grep sed gawk gcompat libc6-compat ca-certificates
}

ensure_glibc() {
  if [ -e /lib/ld-linux-x86-64.so.2 ] || [ -e /lib64/ld-linux-x86-64.so.2 ] || [ -e /usr/glibc-compat/lib/ld-linux-x86-64.so.2 ]; then
    return 0
  fi

  case "$(uname -m)" in
    x86_64|amd64)
      GLIBC_PKG="glibc-2.35-r1.apk"
      ;;
    *)
      warn "当前架构 $(uname -m) 暂未自动安装 sgerrand glibc；如仍无法运行，建议改用 Debian/Ubuntu"
      return 0
      ;;
  esac

  info "安装 Alpine glibc 兼容层..."
  wget -q -O /etc/apk/keys/sgerrand.rsa.pub https://alpine-pkgs.sgerrand.com/sgerrand.rsa.pub
  wget -q -O "/tmp/${GLIBC_PKG}" "https://github.com/sgerrand/alpine-pkg-glibc/releases/download/2.35-r1/${GLIBC_PKG}"
  apk add --allow-untrusted "/tmp/${GLIBC_PKG}"
  rm -f "/tmp/${GLIBC_PKG}"
}
...
test_binary() {
  info "检查配置是否可启动..."
  "$SNELL_DIR/snell-server" -c "$SNELL_CONFIG" >/tmp/snell-test.log 2>&1 &
  test_pid=$!
  sleep 1
...
main() {
  ...
  while true; do
    ...
    case "$choice" in
      ...
      4) write_config && test_binary && rc-service snell restart; pause ;;
