#!/bin/bash
# ========================================
# 🐳 Alpine Linux 专用一键 VPS Docker 管理工具
# ========================================
...
docker_install_update() {
    ...
    apk add docker docker-cli bash curl jq grep
...
main_menu() {
    ...
    while true; do
        ...
        echo -e "${BLUE}        --- Alpine Linux 专用版 ---${RESET}"
        ...
        if command -v docker &>/dev/null; then
            ...
            echo -e "${YELLOW}🐳| 防火墙驱动: $(current_iptables)${RESET}"
        else
        ...
        echo -e "${GREEN}15. 重启 Docker 服务${RESET}"
        echo -e "${GREEN}16. Docker 容器实时监控${RESET}"
        echo -e "${GREEN} 0. 退出${RESET}"
        echo -e "----------------------------------------"
        read -p "$(echo -e ${GREEN}请选择 [0-16]:${RESET}) " choice
        case $choice in
