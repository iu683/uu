...
# =============================================================================
#  Xray VLESS-Reality 智能编号多实例管理面板 
# =============================================================================
...
menu_switch_instance() {
    ...
    echo "目前存储于 ${CONFIG_DIR} 内的独立实例列表:"
    ...
    if [ -d "$CONFIG_DIR" ] && [ "$(ls -A "$CONFIG_DIR")" ]; then
        for f in "${CONFIG_DIR}"/config_*.json; do
            ...
            ((count++))
            local name
            name=$(jq -r '._meta.alias' "$f")
            instance_list+=("$name")
            
            local port_num
            port_num=$(jq -r '.inbounds[0].port' "$f")
...
menu_install() {
    ...
    if [ "$is_edit" = "true" ]; then
        if ! parse_existing_config; then
            die "未检测到实例 [ ${CURRENT_INSTANCE} ] 的旧配置，无法微调，请先按 1 进行全新部署！"
        fi
    ...
    read -r -p "$(echo -e "${GREEN}请输入服务入站端口 [当前: ${YELLOW}${OLD_PORT}${GREEN} | 回车不改]: ${RESET}")" input_port
    ...
    read -r -p "$(echo -e "${GREEN}请输入用户凭证 UUID [当前: ${YELLOW}${OLD_UUID}${GREEN} | 回车不改]: ${RESET}")" input_uuid
    opt_uuid="${input_uuid:-$OLD_UUID}"

    # 3. 伪装 SNI 域名
    local input_domain opt_domain
    read -r -p "$(echo -e "${GREEN}请输入 SNI 伪装域名 [当前: ${YELLOW}${OLD_DOMAIN}${GREEN} | 回车不改]: ${RESET}")" input_domain
    opt_domain="${input_domain:-$OLD_DOMAIN}"

    # 4. ShortID
    local input_sid opt_sid
    read -r -p "$(echo -e "${GREEN}请输入自定义 ShortID [当前: ${YELLOW}${OLD_SHORTID}${GREEN} | 回车不改]: ${RESET}")" input_sid
...
menu_uninstall() {
    ...
    local res
    read -r -p "$(echo -e "${RED}确认抹除清理实例 [ ${CURRENT_INSTANCE} ] 吗？[y/N]: ${RESET}")" res
    [[ "$res" =~ ^[Yy]$ ]] || return
