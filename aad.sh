...
unblock_direct_ip_manual() {
  ...
  for port in $PORTS; do
    rm -f /etc/nginx/sites-available/block_port_$port.conf
    rm -f /etc/nginx/sites-enabled/block_port_$port.conf
  done
  nginx -t && systemctl reload nginx
  echo -e "${GREEN}端口 $PORTS 已解除封禁${RESET}"
  pause
}
...
EOF
  ln -sf "$CONFIG" /etc/nginx/sites-enabled/
  nginx -t && systemctl reload nginx
  echo -e "${GREEN}已禁止 VPS IP 直连 (IPv4: $VPS_IPV4 IPv6: $VPS_IPV6)${RESET}"
  pause
}

allow_vps_ip_access() {
  rm -f /etc/nginx/sites-available/block_vps_ip.conf
  rm -f /etc/nginx/sites-enabled/block_vps_ip.conf
  nginx -t && systemctl reload nginx
  echo -e "${GREEN}已允许 VPS IP 直连${RESET}"
  pause
}
