...
docker_backup_restore() {
    ...
    if ! command -v jq >/dev/null 2>&1; then
        echo "未检测到 jq，正在安装..."
        apk add --no-cache jq
        command -v jq >/dev/null 2>&1 || { echo "jq 安装失败，请手动安装"; return; }
    fi
    ...
    while true; do
        ...
        case "$choice" in
            1)
                # 备份
                echo "选择备份类型: 1. 容器 2. 镜像 3. 卷 4. 全量"
                read -p "请输入选择: " btype
                read -p "备份文件名 (默认 docker_backup_$(date +%F).tar.gz): " backup_name
                ...
                if [ "$btype" = "1" ] || [ "$btype" = "4" ]; then
                    ...
                    read -p "要备份的容器名（留空则全部）: " selected_containers
                    ...
                    for cname in $selected_containers; do
                        ...
                        docker commit "$cid" "${cname}_backup" >/dev/null
                ...
                if [ "$btype" = "2" ] || [ "$btype" = "4" ]; then
                    ...
                    read -p "要备份的镜像（留空则全部）: " selected_images
                ...
                if [ "$btype" = "3" ] || [ "$btype" = "4" ]; then
                    echo "可用卷列表:"
                    docker volume ls -q
                    read -p "要备份的卷名（留空则全部）: " selected_volumes
            ...
            2)
                ...
                for cjson in /tmp/docker_restore/docker_backup/container_*.json; do
                    ...
                    # 加载镜像
                    img_tar="/tmp/docker_restore/docker_backup/image_${cname}.tar"
                    ...
                    cmd="docker run -d --name $cname $envs $ports $mounts --network $network $image_name"
                    echo "恢复容器: $cname"
                    echo "$cmd"
                    $cmd
                done
