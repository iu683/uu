...
function show_all_containers_status() {
    clear
    echo -e "${GREEN}=== 项目: $PROJECT_DIR 所有容器状态 ===${RESET}"
...
function project_menu() {
    while true; do
        ...
        echo -e "${GREEN}11) 一键查看所有容器运行状态${RESET}"
        echo -e "${GREEN} 0) 返回主菜单${RESET}"
        read -p "请选择操作: " choice
        case "$choice" in
