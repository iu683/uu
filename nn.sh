...
update_app() {
    ...
    echo -e "${YELLOW}🔄 开始更新仓库...${RESET}"
    git reset --hard
    git pull

    echo -e "${YELLOW}🛠️ 构建最新镜像...${RESET}"
    docker compose build

    echo -e "${YELLOW}♻️ 停止并更新容器（保留数据卷）...${RESET}"
    # 确保使用 docker compose up -d 保留 volumes
    docker compose up -d

    echo -e "${GREEN}✅ MailAggregator_Pro 更新完成（数据保留）${RESET}"
    echo -e "${GREEN}📂 数据依然在卷或宿主目录: ${DATA_DIR}${RESET}"
    read -p "按回车返回菜单..."
}
restart_app() {
