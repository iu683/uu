...
services:
  alist-tvbox:
    ...
    volumes:
      ...
      - ${BASE_DIR}/www-static:/www/static
EOF
            local show_msg="${YELLOW}管理后台地址 : http://127.0.0.1:${custom_mgt}\nAList 访问地址: http://127.0.0.1:${custom_alist}\n纯净版进 AList 后台用户名：atv (密码在高级设置中查看)${RESET}"
            ;;
    esac
    ...
    if [ -f "$BASE_DIR/initial_admin_credentials.txt" ]; then
        ...
        echo -e "${YELLOW}默认初始密码 : 查看opt/alist-tvbox/initial_admin_credentials.txt${RESET}"
    fi
    echo -e "${GREEN}================================${RESET}"
