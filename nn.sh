...
inst_site(){
    read -rp "请输入 Hysteria 2 的伪装网站地址(默认en.snu.ac.kr)：" proxysite
...
insthysteria(){
    ...
    wget -N https://raw.githubusercontent.com/iu683/uu/main/install_server.sh
...
EOF
    ...
    if [[ -n $(systemctl status hysteria-server 2>/dev/null | grep -w active) && -f '/etc/hysteria/config.yaml' ]]; then
        ...
        red "Hysteria 2 服务启动失败，请运行 systemctl status hysteria-server" && exit 1
    fi
    red "======================================================================================"
    ...
    if [[ -n "$firstport" && -n "$endport" ]]; then
        ...
        yellow "Hysteria 2 端口模式: 单端口 $port"
    fi
...
hysteriaswitch(){
    yellow "请选择你需要的操作："
    echo -e " ${GREEN}1.启动 Hysteria 2${PLAIN}"
    echo -e " ${GREEN}2.关闭 Hysteria 2${PLAIN}"
    echo -e " ${GREEN}3.重启 Hysteria 2${PLAIN}"
...
changeconf(){
    ...
    echo -e " ${GREEN}1.修改端口${PLAIN}"
    echo -e " ${GREEN}2.修改密码${PLAIN}"
    echo -e " ${GREEN}3.修改证书类型${PLAIN}"
    echo -e " ${GREEN}4.修改伪装网站${PLAIN}"
