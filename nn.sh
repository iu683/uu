...
create_user_if_needed() {
  if ! id -u "$SNELL_USER" >/dev/null 2>&1; then
    adduser -S -D -H -s /sbin/nologin "$SNELL_USER"
  fi
}
...
EOF
  ...
  chown -R "$SNELL_USER":"$SNELL_USER" "$SNELL_DIR"
...
EOF
  # 修正 pidfile，避免 heredoc 里被 shell 续行污染
  sed -i 's#pidfile=.*#pidfile="/run/${RC_SVCNAME}.pid"#' "$SNELL_SERVICE"
