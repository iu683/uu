...
if [ -f "$ADMIN_SCRIPT" ]; then
    ...
    curl -sL https://raw.githubusercontent.com/iu683/uu/main/nn.sh > "$ADMIN_SCRIPT"
...
show_status_and_info() {
    ...
    if [ "$timer_active" == "运行中 (Active)" ]; then
        ...
        if [ -n "$raw_line" ]; then
            # 提取前段的时间信息段
            local part_next=$(echo "$raw_line" | awk -F"${SERVICE_NAME}.timer" '{print $1}' | awk '{print $1,$2,$3,$4,$5}')
            local part_last=$(echo "$raw_line" | awk -F"${SERVICE_NAME}.timer" '{print $1}' | awk '{print $(NF-3),$(NF-2),$(NF-1),$NF}')
            
            next_run=$(parse_systemd_time "$part_next")
            last_run=$(parse_systemd_time "$part_last")
        fi
    fi
