        ...
        case $choice in
            ...
            2)
                ...
                while true; do
                    ...
                    # 检测 Docker Compose
                    COMPOSE_YML=$(find "$TMP_RESTORE_DIR" -name "docker-compose.yml" | head -n1)
                    if [[ -n "$COMPOSE_YML" ]]; then
                        echo "检测到 Docker Compose 项目，使用 docker-compose 恢复容器..."
                        PROJECT_DIR=$(dirname "$COMPOSE_YML")
                        pushd "$PROJECT_DIR" >/dev/null
                        docker-compose up -d
                        popd >/dev/null
                        echo "$(date '+%F %T') Docker Compose 项目恢复完成" >> "$LOG_FILE"
                    fi
                    ...
                    if [[ "$rtype" == "3" || "$rtype" == "4" ]]; then
                        for vol_file in "$TMP_RESTORE_DIR"/volume_*.tar.gz; do
                            ...
                            tar -xzf "$vol_file" -C /var/lib/docker/volumes/"$vol_name"/_data --numeric-owner
