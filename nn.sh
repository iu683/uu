...
install_warp() {
    ...
    if [ "$has_v4" -ne 1 ] && [ -f "$CONF_FILE" ]; then
        echo -e "${BLUE}[信息]${RESET} 检测到纯 IPv6 环境，正在通过硬编码清洗 Endpoint 字段..."
        
        # 官方最稳的纯 IPv6 Anycast 节点 IP（不带方括号，不带端口，完全符合它原本的 JSON 格式）
        local pure_cf_v6="2606:4700:103::2" 
        # 或者是这个： "2606:4700:d0::a25c:bc2e"
        
        # 精准替换：把所有 v4 的、旧 v6 的值，全部强制格式化为好使的 IPv6 物理地址，且完美保留尾部逗号
        ...
        echo -e "${GREEN}[成功]${RESET} 全新安装下的 IPv6 终极精准重写已完成。"
        echo -e "${BLUE}[当前实际写入的配置文件核对]:${RESET}"
        cat "$CONF_FILE"
    fi
