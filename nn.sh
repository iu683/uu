...
require_root() { [[ $(id -u) -eq 0 ]] || { _err "请以 root 身份运行"; exit 1; }; }
require_alpine() { [[ -f /etc/alpine-release ]] || { _err "此脚本仅适用于 Alpine Linux"; exit 1; }; }
...
_has_ipv6() { ip -6 addr show scope global 2>/dev/null | grep -q 'inet6'; }
_can_dual_stack_listen() { [[ -r /proc/sys/net/ipv6/bindv6only ]] && [[ "$(cat /proc/sys/net/ipv6/bindv6only 2>/dev/null || echo 1)" == "0" ]]; }
_listen_addr() { _has_ipv6 && _can_dual_stack_listen && echo "::" || echo "0.0.0.0"; }
_fmt_hostport() { local host="$1" port="$2"; [[ "$host" == *:* ]] && printf '[%s]:%s' "$host" "$port" || printf '%s:%s' "$host" "$port"; }

check_glibc_compat() {
  if /lib/ld-musl-x86_64.so.1 --help >/dev/null 2>&1; then
    :
  fi
  if [[ -e /lib/libc.so.6 || -e /usr/glibc-compat/lib/libc.so.6 || -e /lib64/ld-linux-x86-64.so.2 ]]; then
    return 0
  fi
  return 1
}
...
install_glibc_compat() {
  _warn "检测到可能缺少 glibc 兼容库，正在尝试安装..."
  if check_cmd apk; then
    apk add --no-cache libc6-compat gcompat >/dev/null 2>&1 || true
  fi
  if ! check_glibc_compat; then
    _warn "glibc 兼容层仍不可用；Snell v5 可能无法在纯 Alpine 下运行。"
  else
    _ok "glibc 兼容层检查通过"
  fi
}
...
ensure_deps() {
  apk add --no-cache curl unzip openssl bash iproute2 file gcompat libc6-compat >/dev/null
  check_cmd upx || apk add --no-cache upx >/dev/null 2>&1 || true
  if ! check_glibc_compat; then
    install_glibc_compat
  fi
}
...
_get_snell_latest_version() { local v; v=$(_get_snell_versions_from_kb 1 | head -n1); [[ -z "$v" ]] && v="$DEFAULT_VERSION"; echo "$v"; }
...
install_snell_v5() {
  local channel="${1:-stable}" force="${2:-false}" version_override="${3:-}"
  local exists=false action="安装" channel_label="稳定版"
  if check_cmd snell-server-v5; then exists=true; [[ "$force" != "true" ]] && { _ok "Snell v5 已安装"; return 0; }; fi
  [[ "$exists" == "true" ]] && action="更新"
  if [[ "$channel" == "prerelease" || "$channel" == "test" || "$channel" == "beta" ]]; then _warn "Snell v5 未提供预发布版本，使用稳定版"; channel="stable"; fi
  local sarch version tmp url
  sarch=$(_map_arch) || { _err "不支持的架构"; return 1; }
  if [[ -n "$version_override" ]]; then _info "$action Snell v5 (版本 v$version_override)..."; version="$version_override"; else _info "$action Snell v5 (获取最新${channel_label})..."; version=$(_get_snell_latest_version); fi
...
gen_snell_link() { local ip="$1" port="$2" psk="$3" version="${4:-5}" name="${5:-Snell-v${version}}"; printf 'snell://%s@%s:%s?version=%s#%s\n' "$psk" "$ip" "$port" "$version" "$name"; }
...
restart_service() { svc restart "$SERVICE"; rc-service "$SERVICE" status || true; }
...
show_versions() { local current latest; current=$(_get_snell_v5_version); latest=$(_get_snell_latest_version); echo "----------------------------------------"; echo -e "  ${W}Snell v5${NC}"; echo -e "    当前版本: ${G}${current}${NC}"; echo -e "    稳定版本: ${C}${latest}${NC}"; echo "----------------------------------------"; }
show_changelog() { local version; version=$(_get_snell_latest_version); echo "----------------------------------------"; echo "Snell v5 最新版本: $version"; echo "----------------------------------------"; _get_snell_changelog_from_kb "$version" || true; echo "----------------------------------------"; }
...
interactive_install() {
  ...
  port=${port:-443}
  if ! [[ "$port" =~ ^[0-9]+$ ]] || (( port < 1 || port > 65535 )); then _err "端口无效"; return 1; fi
...
pause_wait() { read -n 1 -s -r -p "按任意键继续..." || true; echo; }

menu() {
  while true; do
    clear
    echo -e "${C}Snell v5 Alpine 提取版${NC}"
    echo "----------------------------------------"
...
main() { require_root; require_alpine; menu; }
main "$@"
