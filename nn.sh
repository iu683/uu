...
install_packages() {
  ...
  # 【修复核心 1】：强行启用 Alpine 防火墙持久化服务，并将其加入开机自启
...
apply_new_iptables() {
  ...
  if [[ -f "${CONFIG_DIR}/hopping.txt" ]]; then
    ...
    # 【修复核心 2】：强制将最新的链条持久化保存到 Alpine 系统防火墙配置文件中
    if [[ -f /etc/init.d/iptables ]]; then
      /etc/init.d/iptables save &>/dev/null || true
    fi
    if [[ -f /etc/init.d/ip6tables ]]; then
      /etc/init.d/ip6tables save &>/dev/null || true
    fi
    info "防火墙端口跳跃规则已固化，重启不再失效。"
  fi
}
...
depend() {
    need net
    # 【修复核心 3】：必须显式声明在防火墙服务启动完成之后，再去加载 sing-box-tuic
