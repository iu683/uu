...
install_nginx() {
    if command -v nginx >/dev/null 2>&1 && command -v certbot >/dev/null 2>&1; then
        echo -e "${YELLOW}提示: 检测到系统已安装 Nginx 与 Certbot，自动跳过安装流程。${RESET}"
        pause
        return
    fi
...
add_custom_cert_config() {
    ...
    if nginx -t; then
        ...
        echo -e "${RED}Nginx 配置测试失败，请检查自定义证书。${RESET}"
...
check_cert() {
    ...
    echo -e "${GREEN}2) 查看本地自定义证书目录${RESET}"
...
update_nginx_software() {
    ...
    echo -ne "${YELLOW}是否要从官方源拉取检查并平滑升级？(y/N): ${RESET}"
    read up_choice
    ...
    local BACKUP_DIR="/root/nginx_conf_backup_$(date +%Y%m%d)"
    mkdir -p "$BACKUP_DIR"
