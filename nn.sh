...
install_utils() {
    ...
    # 💡 关键修改：去掉了 -t 参数，只保留 -i 供交互输入密码
    docker run -i --rm ghcr.io/steveiliop56/tinyauth:v4 user create --interactive | tee "$tmp_log"

    # 💡 优化提取：先用 sed 剥离可能存在的 ANSI 颜色字符，再进行精准提取
    local extracted_user=$(sed -r "s/\x1B\[([0-9]{1,2}(;[0-9]{1,2})?)?[mGK]//g" "$tmp_log" | grep -a "User created user=" | awk -F'user=' '{print $2}' | tr -d '\r' | tr -d '\n')
    rm -f "$tmp_log"

    if [[ -z "$extracted_user" ]]; then
        echo -e "${RED}错误: 未能成功捕获到用户 Hash！${RESET}"
        # 💡 提示优化：如果你希望用户手动输入时不用考虑 Docker 的 $$ 转义，
        # 可以在这里做个提示，因为用户输入的是纯文本，后续写入 env 时脚本可以用 sed 把 $ 替换为 $$
        ...
        # 如果用户手动输入了，自动帮他把单 $ 转换为双 $$ 防止 docker-compose 报错
        extracted_user=$(echo "$extracted_user" | sed 's/\$/$$/g')
    else
        # 💡 关键修复：如果是脚本自动捕获的，也要把单 $ 转换为双 $$
        extracted_user=$(echo "$extracted_user" | sed 's/\$/$$/g')
    fi
