...
update_core_binary() {
    ...
    if [ -z "$latest_version" ] || [ -z "$download_url" ]; then
        error "无法从 GitHub 获取版本信息，请检查 network。"
        return 1
    fi
...
install_tun2socks() {
    ...
    if [ -n "$MAIN_IP6" ]; then
        # 【此处已修复Bug】这里之前误写成了 RULE_DEL_FROM_MAIN_IP6="..."
...
EOF
    ...
    systemctl start tun2socks.service || { 
        error "启动服务失败！请在退出菜单后执行 'journalctl -u tun2socks.service -n 20' 查看具体错误日志。"
        return 1
    }
