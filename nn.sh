...
function install_app() {
    ...
    read -p "监听端口 [默认: 3301]: " PORT

    BRIDGE_TOKEN=$(openssl rand -hex 32)
    echo -e "${GREEN}已自动生成 Bridge Token: ${BRIDGE_TOKEN}${RESET}"
    ...
    echo -e "${YELLOW}默认登录: ${APP_LOGIN_USERNAME} / ${APP_LOGIN_PASSWORD}${RESET}"
...
function app_status() {
    ...
    ss -tulnp | grep -E ":$(grep '^PORT=' .env | cut -d= -f2)" || true
