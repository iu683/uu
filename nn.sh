...
inst_cert() {
    ...
    echo "---------------------------------------------"
    echo -e "AnyTLS 证书与域名配置方式如下："
    echo -e " 1) 必应自签模式 ${YELLOW}（默认 - 客户端跳过证书验证）${RESET}"
    echo -e " 2) Acme 脚本申请 (获取正规域名，自动同步 SNI)"
    echo -e " 3) 绑定已有自定义证书路径 (自动提取 SNI 域名)"
    echo "---------------------------------------------"
    local certInput
    ...
    if [[ $certInput == 1 ]]; then
        echo -e "${GREEN}[信息] 将使用必应自签证书模式${RESET}"
...
write_systemd_service() {
    # 已移除 --cert 和 --key 参数，完全对齐 anytls-server 原生参数接口
...
modify_ss() {
    ...
    echo "---------------------------------------------"
    read -rp "是否需要修改证书或重设域名 SNI？[y/N] (直接回车默认不修改): " change_cert_flag < /dev/tty
