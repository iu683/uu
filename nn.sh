...
get_system_status() {
    if [ -f "$ACME_HOME/acme.sh" ]; then
        ...
        # 兜底策略精准抓取版本号
        VERSION_SHOW=$($ACME_HOME/acme.sh --version 2>/dev/null | head -n 1 | awk '{print $2}' | tr -d '[:space:]')
        
        # 如果 --version 拿不到，直接跨过接口去分析文本源码中的关键字
        if [ -z "$VERSION_SHOW" ] || [ "$VERSION_SHOW" = "v" ]; then
            VERSION_SHOW=$(grep -E '^_VERSION=' "$ACME_HOME/acme.sh" | cut -d'"' -f2 | cut -d"'" -f2 | head -n 1)
        fi
        
        # 如果连源码都分析不到，提取文件修改日期作为动态版本号
        if [ -z "$VERSION_SHOW" ]; then
            VERSION_SHOW=$(date -r "$ACME_HOME/acme.sh" "+%Y.%m.%d" 2>/dev/null || echo "v3.0+稳定版")
        fi
