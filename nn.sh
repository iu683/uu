...
fetch_latest_version() {
    ...
    info "正在获取最新 Release 版本号..."
    TMP_API="$(mktemp)"
    
    # 针对 API 请求，如果是镜像站通常也支持转发，这里直接通过代理请求 API
    ...
    if [ -z "$VERSION" ]; then
        ...
        VERSION=$(curl -sS --connect-timeout 5 "${SELECTED_PROXY}https://github.com/${REPO}/releases/latest" 2>/dev/null | grep -o 'tag/[vV]*[0-9.]*' | awk -F '/' 'NR==1 {print $2}')
    fi
