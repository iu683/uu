...
update_singbox() {
  ...
  info "当前版本: ${YELLOW}${current_version}${RESET} | 最新版本: ${GREEN}${latest_version}${RESET}"
...
configure_custom_socks5_outbound() {
    ...
    current_protocol=$(jq -r '.outbounds[0].type // "freedom"' "$instance_config" 2>/dev/null || echo "freedom")
    ...
    case "$mode" in
        1)
            tmp_file=$(mktemp)
            jq '.outbounds = [{"type":"freedom","tag":"direct"}]' "$instance_config" > "$tmp_file"
