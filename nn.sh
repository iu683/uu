...
register_usque() {
    local has_v4=0
    if check_v4_connectivity; then has_v4=1; fi
    ...
    if "${reg_cmd[@]}"; then
        ...
        # 🌟 核心修复：针对纯 IPv6 环境执行极限制胜的“两端点镜像洗刷”
        if [ -f "$CONF_FILE" ]; then
            if [ "$has_v4" -ne 1 ]; then
                info "正在对纯 IPv6 环境进行核心配置文件 [双端点镜像同步] 清洗..."
                
                # 强行将 endpoint_v4 和 endpoint_v6 全部锁定为官方 v6 任播地址
                sed -i 's/"endpoint_v4": *"[^"]*"/"endpoint_v4": "2606:4700:102::1"/g' "$CONF_FILE"
                sed -i 's/"endpoint_v6": *"[^"]*"/"endpoint_v6": "2606:4700:102::1"/g' "$CONF_FILE"
                
                # 兜底防呆：如果发现内核认标准单独的 "endpoint" 字段，同步改写
                if grep -q '"endpoint"' "$CONF_FILE"; then
                    sed -i -E 's/"endpoint": *"[^"]*"/"endpoint": "[2606:4700:102::1]:2408"/g' "$CONF_FILE"
                fi
                
                ok "清洗成功：[endpoint_v4] 与 [endpoint_v6] 已全部焊死重定向至 -> 2606:4700:102::1"
            else
                ok "双栈/V4 环境验证通过，保留官方原生端点配置，未做任何修改。"
            fi
        fi
    else
