#!/usr/bin/env bash
#
# nftables 端口转发管理工具 (DDNS 修复日志菜单最终版)
#
...
# 【核心修复】：移除易受隐形字符干扰的 awk，改用 sed 强力剥离，防 IP 末位截断
resolve_domain() {
    local domain="$1"
    local resolved=""
    if command -v nslookup &>/dev/null; then
        resolved=$(nslookup "$domain" 8.8.8.8 2>/dev/null | sed -n 's/^Address:[[:space:]]*\([0-9.]*\).*$/\1/p' | head -n1 | tr -d '\r\n[:space:]')
...
# 【核心修复】：由于后台环境缺乏 PATH 变量，全部强制统一使用绝对路径 /usr/sbin/nft
reload_rules() {
