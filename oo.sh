...
EOF
    ...
    echo -e "${YELLOW}等待容器初始化并生成随机初始密码 (约8秒)...${RESET}"
    # 稍微延长等待时间，确保日志已经完整打印
    sleep 8
    ...
    # 优化后的抓取逻辑：
    # 1. 先尝试用 ➜ 符号和 password 关键字精准匹配
    # 2. 如果没匹配到，则打印包含 WebUI 提示的上下文 10 行
    if docker logs "$CONTAINER_NAME" 2>&1 | grep -q "Initial password"; then
        docker logs "$CONTAINER_NAME" 2>&1 | grep -A 2 -B 3 "Initial password"
    else
        docker logs --tail 30 "$CONTAINER_NAME" 2>&1 | grep -A 10 "WebUI is ready"
    fi
    echo -e "${CYAN}--------------------------------${RESET}"
}
