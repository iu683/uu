...
EOF
    ...
    # 【核心逻辑修正点】
    # 如果端口不是 8080，等待容器把配置目录生成出来，随后立马动态注入热补丁
    if [[ "$custom_port" != "8080" ]]; then
        echo -e "${YELLOW}正在等待容器初始化并创建初始配置文件...${RESET}"
        CONF_FILE="$BASE_DIR/config/qBittorrent/qBittorrent.conf"
        
        # 循环探测配置文件是否被拉起（最多等待 15 秒）
        for i in {1..15}; do
            if [[ -f "$CONF_FILE" ]]; then
                break
            fi
        ...
        if [[ -f "$CONF_FILE" ]]; then
            echo -e "${YELLOW}检测到自定义端口 [${custom_port}]，正在动态注入防 Unauthorized 补丁...${RESET}"
            # 确保有 Preferences 节点
            ...
            if ! grep -q "WebUI\\\HostHeaderValidation" "$CONF_FILE"; then
                ...
                sed -i 's/WebUI\\HostHeaderValidation=true/WebUI\\HostHeaderValidation=false/g' "$CONF_FILE"
            fi
            # 重启使其生效
            echo -e "${YELLOW}正在重启容器应用安全新配置...${RESET}"
            docker restart "$CONTAINER_NAME" >/dev/null
        else
            echo -e "${RED}警告：未能在预定时间内检测到初始配置文件，请在稍后登录异常时手动执行选项 4 重启容器。${RESET}"
        fi
    fi
