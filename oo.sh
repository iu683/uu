...
write_config() {
    ...
    # 【逻辑顺序完美修正】：优先判定是否包含双栈特征，最后才是单栈兜底
    local real_listen=""
    case "$listen_mode" in
        *"0.0.0.0"*":""*"[::]"*|*"dual"*) 
            real_listen="0.0.0.0:${port},[::]:${port}" ;; # 完美的双栈匹配
        *"[::]"*)    
            real_listen="[::]:${port}" ;;                 # 仅 IPv6
        *"0.0.0.0"*)  
            real_listen="0.0.0.0:${port}" ;;              # 仅 IPv4
        *)           
            real_listen="0.0.0.0:${port},[::]:${port}" ;; # 新建节点默认双栈
    esac
