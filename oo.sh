...
inst_cert() {
    ...
    if [[ ! -f "$acme_cmd" ]]; then
        ...
        info "正在从官方源下载并安装 acme.sh ..."
        # 直接连接 GitHub 官方 Raw 站
        if curl -fsSL --max-time 15 "https://raw.githubusercontent.com/acmesh-official/acme.sh/master/acme.sh" -o "$TMP_DIR/acme_install.sh"; then
