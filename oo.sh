...
# 动态获取核心容器状态与映射端口
get_status_info() {
    local active_id=$(docker ps -q -f name=^/web$ || docker ps -q -f name=^/rhex-web$ || docker ps -q --filter "name=_web_")
    if [ -n "$active_id" ]; then
        status="${GREEN}运行中${RESET}"
        webui_port=$(docker inspect --format='{{(index (index .NetworkSettings.Ports "3000/tcp") 0).HostPort}}' "$active_id" 2>/dev/null)
        ...
        local dead_id=$(docker ps -aq -f name=^/web$ || docker ps -aq -f name=^/rhex-web$ || docker ps -aq --filter "name=_web_")
