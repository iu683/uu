...
if [ -n "$REAL_CMD" ] || [ -d "/opt/1panel" ]; then
    ...
    # 2. 强读版本号（增加直接查询 /usr/local/bin/1panel 版本的逻辑）
    local ver_info=""
    if [ -x "/usr/local/bin/1panel" ]; then
    ...
    if [ -z "$ver_info" ] && [ -n "$REAL_CMD" ]; then
        ver_info=$($REAL_CMD version 2>/dev/null | grep -i "version" | awk '{print $2}')
    fi
    echo -e "当前版本: ${YELLOW}v${ver_info:-2.x (已安装)}${RESET}"
    ...
    if [ -z "$port" ]; then
        port=$(netstat -tuln 2>/dev/null | grep -E "1panel|1p-" | awk '{print $4}' | awk -F':' '{print $NF}')
    fi
