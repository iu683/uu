#!/bin/bash
# =================================================================
# Bomb Party Docker Compose 管理面板 
# =================================================================
...
CONTAINER_NAME="bomb-party"
BASE_DIR="/opt/bomb-party"
...
services:
  bomb-party:
    image: kjlion/bomb-party:latest
...
EOF
    ...
    echo -e "${YELLOW}正在通过 Docker Compose 启动 Bomb Party 服务...${RESET}"
    ...
    echo -e "${GREEN}====================================================${RESET}"
    echo -e "${GREEN}          Bomb Party 部署及启动成功！               ${RESET}"
    echo -e "${GREEN}====================================================${RESET}"
...
# 卸载服务
uninstall_party() {
    echo -ne "${YELLOW}确定要卸载并删除 Bomb Party 容器吗？(y/n): ${RESET}"
    read -r confirm
...
start_party() { cd "$BASE_DIR" && docker compose start && echo -e "${GREEN}容器已启动${RESET}"; }
stop_party() { cd "$BASE_DIR" && docker compose stop && echo -e "${YELLOW}容器已停止${RESET}"; }
restart_party() { cd "$BASE_DIR" && docker compose restart && echo -e "${GREEN}容器已重启${RESET}"; }
logs_party() { 
...
menu() {
    ...
    get_status_info
    echo -e "${GREEN}==============================${RESET}"
    echo -e "${GREEN}    ◈  Bomb Party 面板  ◈    ${RESET}"
    echo -e "${GREEN}==============================${RESET}"
