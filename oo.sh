...
# ==============================================================================
#   Usque (MASQUE-WARP) 全能综合控制面板 (含 Google 透明代理 与 Hev-Tunnel 引擎)
# ==============================================================================
...
update_hev_core() {
    if [ ! -f "$HEV_BIN" ]; then
        error "尚未检测到 Tun2Socks(Hev) 运行环境，请先执行选项 1 部署。"
        return 1
    fi
...
menu_hev_tunnel_center() {
    while true; do
        ...
        if systemctl is-active --quiet "$HEV_SERVICE_NAME"; then status_show="${GREEN}已启动 (三层全局托管中)${RESET}"; fi
...
while true; do
    ...
    echo -e "${YELLOW} 10. 谷歌分流${RESET}"
    echo -e "${YELLOW} 11. Tun2Socks全局出口${RESET}"
    echo -e "$GREEN}  0. 退出${RESET}"
    echo -e "${GREEN}==============================${RESET}"
    ...
    case "$choice" in
        ...
        3) 
            ...
            ok "已彻底将核心组件、Google透明代理及Hev-Tunnel三层网卡组件全部清理干净。"
            sleep 0.5 
            ;;
        4) menu_edit_config ;;
