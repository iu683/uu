#!/bin/bash
# =================================================================
# MTProto (mtg) 代理 Docker Compose 多节点管理面板 (Surge/v2 兼容版)
# =================================================================
...
install_utils() {
    ...
    # 2. 生成标准的 32 位 Hex 密钥（服务端专用）
    echo -e "${YELLOW}正在生成标准 32 位 Hex 密钥...${RESET}"
...
EOF
    ...
    # 4. 生成 config.toml (注意：服务端配置只能填纯 Hex，不能加 dd)
    cat > "$CONFIG_FILE" <<EOF
secret = "$RAW_SECRET"
...
show_info() {
    ...
    if [ -f "$ENV_FILE" ]; then
        ...
        # 兼容旧配置无 CLIENT_SECRET 的逻辑
        [[ -z "$MTG_CLIENT_SECRET" ]] && MTG_CLIENT_SECRET="dd${MTG_SECRET}"
