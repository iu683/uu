...
# ==============================================================================
#  MicaProxy 进阶实例化安全一键管理面板 (完美沙盒修复版)
# ==============================================================================
...
write_hardened_systemd() {
    # ── 🔥 核心修复点 ──
    # 将原来的 ReadOnlyPaths 细化规则调整为直接放行 /etc/MicaProxy/instances/ 整个配置目录
    # 彻底解决 Systemd 在部分系统沙盒上无法正确在声明区内解析 %%i 文件路径导致 os error 2 的 Bug
...
# 路径沙盒可见性控制（直接放行父级配置目录）
ReadWritePaths=${LOG_DIR} ${DATA_DIR}
ReadOnlyPaths=${INSTALL_BIN} ${BASE_CONF_DIR}/
...
while true; do
    ...
    echo -e "${GREEN} 0. Safe Quit (安全退出)${RESET}"
    echo -e "${GREEN}===========================================${RESET}"
