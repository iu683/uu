...
inst_cert() {
    ...
    if [[ $certInput == 2 ]]; then
        ...
        if [[ ! -f "$acme_cmd" ]]; then
            ...
            # 1. 干净利落地把标准单文件下载到临时目录
            if curl -fsSL "https://raw.githubusercontent.com/acmesh-official/acme.sh/master/acme.sh" -o "$TMP_DIR/acme_install.sh"; then
                if [ -s "$TMP_DIR/acme_install.sh" ]; then
                    info "下载成功，正在开始本地无依赖安装..."
                    # 2. 绕过 get.acme.sh 的传参 Bug，直接本地 sh 执行，参数绝对不会再变型
