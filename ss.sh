...
get_status_info() {
    ...
    if [ "$(docker ps -aq -f name=^/${CONTAINER_NAME}$)" ]; then
        # 抓取 build_version 并剔除前面的 "Linuxserver.io version: " 字段，只留下版本号数字
        img_version=$(docker inspect -f '{{ index .Config.Labels "build_version" }}' "$CONTAINER_NAME" 2>/dev/null | sed 's/Linuxserver.io - //g' | awk '{print $1}')
