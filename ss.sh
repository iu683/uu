...
install_xbctl() {
    ...
    # 1. 选择模式
    read -p "$(echo -e " 请选择安装模式 [1) Node 模式 | 2) Machine 模式, 默认1]: ${PLAIN}")" install_mode_choice
...
modify_config() {
    echo -e "\n${YELLOW}========== 修改/初始化配置 ==========${PLAIN}"
    
    # 1. 选择模式
    read -p "$(echo -e " ${GREEN}请选择绑定模式 [1) Node 模式 | 2) Machine 模式, 默认1]: ${PLAIN}")" mode_choice
