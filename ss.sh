...
# =========================================================
# Hysteria 2 管理脚本 (Alpine Linux 专用)
# =========================================================
...
get_version() {
    if [[ -x "$HY_BIN" ]]; then
        "$HY_BIN" version 2>/dev/null | awk '/version/ {print $3}' || echo "未知"
    else echo "未安装"; fi
}
...
manage_udp_jump() {
    ...
    if [ "$action" == "add" ]; then
        ...
        info "UDP 跳跃规则已添加: ${start}:${end} -> ${target_port}"
    fi
}
...
EOF
    ...
    # --- 端口跳跃设置开始 ---
    local current_main=$(grep 'listen:' "$HY_CONFIG" | cut -d':' -f3)
    ...
    if [[ -n "$firstport" ]]; then
        ...
        if [[ "$endport" -gt "$firstport" ]]; then
            manage_udp_jump "add" "$firstport" "$endport" "$current_main"
        else
            error "末尾端口必须大于起始端口，跳过规则设置。"
        fi
    fi
    # --- 端口跳跃设置结束 ---
    ...
    echo "hysteria2://$pass@$ip:$current_main/?insecure=1&sni=www.bing.com#$(hostname)" > "$HY_NODE_FILE"
    info "操作成功完成！"
}
...
while true; do
    ...
    case $choice in
        ...
        3) rc-service hysteria stop 2>/dev/null || true; manage_udp_jump "remove"; rm -rf "$HY_DIR" "$HY_BIN" /etc/init.d/hysteria "$HY_LOG"; info "彻底卸载完成"; pause ;;
        4) install_hy2 2; pause ;; # 修改配置复用安装逻辑中带提示的部分
        ...
        9) [[ -f "$HY_NODE_FILE" ]] && info "链接: ${YELLOW}$(cat "$HY_NODE_FILE")${RESET}" || error "无配置"; pause ;;
        0) exit 0 ;;
        *) error "无效选项"; sleep 1 ;;
    esac
done
