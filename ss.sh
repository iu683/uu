#!/bin/bash
# ========================================
# qBittorrent-Nox (SourceForge 纯静态最新版) 一键脚本
# ========================================
...
# 自动解析官网静态编译分发线上的最新版本链接（绕过 AppImage，直奔纯静态）
get_latest_release() {
    echo -e "${YELLOW}正在通过 SourceForge/GitHub 静态链检索最新纯净二进制...${RESET}"
    
    # 官网下载页面指定的 qbittorrent-nox 纯静态二进制（零依赖）维护源
    local release_json
    release_json=$(curl -s --max-time 10 https://api.github.com/repos/user-vividlee/qBittorrent-Build-Artifacts/releases/latest)
    
    if [[ -z "$release_json" ]] || [[ "$release_json" == *"message"* ]]; then
        echo -e "${RED}错误: 无法获取最新 Release 信息，请检查网络。${RESET}"
        return 1
    fi

    LATEST_TAG=$(echo "$release_json" | grep -m1 '"tag_name":' | sed -E 's/.*"tag_name":[^"]*"([^"]+)".*/\1/')
    # 精准拉取没有图形渲染库残留的末尾带 _static 的 Linux x86_64 标准包
    ...
    if [[ -z "$DOWNLOAD_URL" || -z "$LATEST_TAG" ]]; then
        echo -e "${RED}错误: 未能解析出服务器专用的静态下载链接！${RESET}"
        return 1
    fi
    
    echo -e "${GREEN}成功匹配纯静态独立版：${LATEST_TAG}${RESET}"
    return 0
}
...
install_qbittorrent() {
    ...
    echo -e "${YELLOW}正在下载专为 Linux 终端环境定制的静态二进制包...${RESET}"
    sudo curl -L -o "$BIN_FILE" "$DOWNLOAD_URL"
    
    if [[ $? -ne 0 || ! -s "$BIN_FILE" ]]; then
        echo -e "${RED}错误: 下载文件失败，请检查机器的网络连接！${RESET}"
        return
    fi
...
[Unit]
Description=qBittorrent Command Line Client (Pure Static)
...
EOF
    ...
    echo -e "${GREEN}================================${RESET}"
    echo -e "${GREEN}qBittorrent-Nox 纯静态版安装完成并已稳定运行!${RESET}"
...
update_qbittorrent() {
    ...
    echo -e "${YELLOW}正在下载最新静态文件覆盖...${RESET}"
    sudo curl -L -o "$BIN_FILE" "$DOWNLOAD_URL"
    chmod +x "$BIN_FILE"
    echo -e "${YELLOW}正在重新启动服务...${RESET}"
    sudo systemctl start "$SERVICE_NAME"
    echo -e "${GREEN}成功自动同步/修复至最新版 (${LATEST_TAG})！${RESET}"
}
...
menu() {
    ...
    get_status_info
    echo -e "${GREEN}================================${RESET}"
    echo -e "${GREEN}   qBittorrent 纯静态管理面板   ${RESET}"
    echo -e "${GREEN}================================${RESET}"
    ...
    echo -e "${GREEN}================================${RESET}"
    echo -e "${GREEN}1. 自动安装 qBittorrent (纯静态免图形依赖版)${RESET}"
