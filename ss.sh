...
install_ddns(){
    if [ ! -f "/usr/bin/ddns" ]; then
        curl -o /usr/bin/ddns  https://raw.githubusercontent.com/iu683/uu/main/ss.sh && chmod +x /usr/bin/ddns
    fi
    mkdir -p /etc/DDNS
...
get_system_status() {
    ...
    if [[ "$ipv6_set" == "true" && ${#Domainsv6[@]} -gt 0 ]]; then
        ...
        SHOW_V6_DOMAINS="${RED}未开启或未配置${RESET}"
    fi
}
