#!/bin/sh
# 完美兼容 Alpine Linux (BusyBox sh) 的双栈防火墙管理脚本
set -e
...
ip_action() {
    ...
    # 安全地清理已存在的相同 IP 规则（防止 set -e 导致脚本闪退）
    set +e
...
# ===============================
# ⚡ 重点修复：Alpine 兼容型 PING (ICMP) 操作函数
# ===============================
ping_action() {
    local action=$1
    
    # 彻底关闭 set -e 以免 BusyBox 的 iptables -C 报错闪退
    set +e
