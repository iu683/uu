...
SCRIPT_URL="https://raw.githubusercontent.com/iu683/uu/main/ss.sh"
...
generate_and_setup_ssh() {
    ...
    # 首次连接自动接受 host key 并静默指纹
    ssh -o StrictHostKeyChecking=no -o UserKnownHostsFile=/dev/null -p "$port" "$remote" "mkdir -p ~/.ssh && chmod 700 ~/.ssh"
    ssh -o StrictHostKeyChecking=no -o UserKnownHostsFile=/dev/null -p "$port" "$remote" "touch ~/.ssh/authorized_keys && chmod 600 ~/.ssh/authorized_keys"
    ssh -o StrictHostKeyChecking=no -o UserKnownHostsFile=/dev/null -p "$port" "$remote" "grep -Fxq '$PUBKEY_CONTENT' ~/.ssh/authorized_keys || echo '$PUBKEY_CONTENT' >> ~/.ssh/authorized_keys"

    # 测试免密
    ssh -o StrictHostKeyChecking=no -o UserKnownHostsFile=/dev/null -i "$KEY_FILE" -p "$port" "$remote" "echo ok" >/dev/null 2>&1
    ok=$?
    set -e
...
add_task() {
    ...
    # 新数据存储拓扑结构：name|local|user|ip|port|auth|secret|remote_path
...
run_task() {
    ...
    # 解析新版拓扑
    IFS='|' read -r name local user ip port auth secret remote_path <<< "$task"
    
    safe_name=$(echo "$name" | tr '/' '_')
    archive="/tmp/sync_task_${safe_name}.tar.gz"
    remote="${user}@${ip}"
    ...
    if [[ "$direction" == "push" ]]; then
        ...
        if [[ "$auth" == "password" ]]; then
            sshpass -p "$secret" ssh -o StrictHostKeyChecking=no -o UserKnownHostsFile=/dev/null -p "$port" "$remote" "mkdir -p $remote_path" && \
            sshpass -p "$secret" rsync -az -e "ssh -o StrictHostKeyChecking=no -o UserKnownHostsFile=/dev/null -p $port" "$archive" "$remote:$remote_path/"
            sync_ok=$?
        else
            ssh -i "$secret" -o StrictHostKeyChecking=no -o UserKnownHostsFile=/dev/null -p "$port" "$remote" "mkdir -p $remote_path" && \
            rsync -az -e "ssh -i $secret -o StrictHostKeyChecking=no -o UserKnownHostsFile=/dev/null -p $port" "$archive" "$remote:$remote_path/"
            sync_ok=$?
        fi
        ...
        local sync_ok=1
        if [[ "$auth" == "password" ]]; then
            sshpass -p "$secret" rsync -az -e "ssh -o StrictHostKeyChecking=no -o UserKnownHostsFile=/dev/null -p $port" "$remote:$remote_path/$(basename "$archive")" "/tmp/"
            sync_ok=$?
        else
            rsync -az -e "ssh -i $secret -o StrictHostKeyChecking=no -o UserKnownHostsFile=/dev/null -p $port" "$remote:$remote_path/$(basename "$archive")" "/tmp/"
            sync_ok=$?
        fi
        ...
        if [ $sync_ok -eq 0 ] && [ -f "$archive" ]; then
            rm -rf "$local"
            mkdir -p "$local"
            tar -xzf "$archive" -C "$(dirname "$local")"
            rm -f "$archive"
...
while true; do
    ...
    case $choice in
        ...
        10) update_self ;;
        11) uninstall_self ;;
        0) echo -e "${YELLOW}正在安全退出系统...${RESET}" ; exit 0 ;;
