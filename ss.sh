...
get_latest_version() {
    ...
    [[ -z "$latest_version" ]] && latest_version="v6.0.0b3"
    echo "$latest_version"
}
...
download_snell_core() {
    ...
    if wget --spider -q -T 5 "$url_v"; then
        ...
        if wget --spider -q -T 5 "$url_no_v"; then
            ...
            echo -e "${RED}[警告] 远程正式版文件未就绪，使用 v6.0.0b3 进行弹性回滚...${RESET}"
            local fallback_url="https://dl.nssurge.com/snell/snell-server-v6.0.0b3-${target_arch}.zip"
            wget -O snell.zip "$fallback_url"
            echo "v6.0.0b3" > "${SNELL_DIR}/version.txt"
        fi
    fi
}
...
show_menu() {
    ...
    local status_stls="${RED}● TLS v3未运行${RESET}"
    systemctl is-active --quiet snell-tlss-v6 && status_snell="${GREEN}● Snell v6运行中${RESET}"
    systemctl is-active --quiet shadowtlsn-v6 && status_stls="${GREEN}● TLS v3运行中${RESET}"
