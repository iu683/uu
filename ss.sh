...
select_available_proxy() {
    ...
    for proxy in $GITHUB_PROXIES; do
        if [ "$proxy" = "DIRECT" ]; then
            # 直连测试：直接请求一次 API 的头部信息
            if curl -I --connect-timeout 3 -m 5 -H "Accept: application/vnd.github+json" "https://api.github.com/repos/${REPO}/releases/latest" >/dev/null 2>&1; then
                export SELECTED_PROXY=""
                ok "检测到当前网络可以直连 GitHub API，将优先使用原生链接。"
                return 0
            fi
        else
            # 代理测试：通过代理去请求 API 的头部信息
            # 这样可以确保筛选出来的代理是【真正能成功转发 API】的
            if curl -I --connect-timeout 3 -m 5 -H "Accept: application/vnd.github+json" "${proxy}https://api.github.com/repos/${REPO}/releases/latest" >/dev/null 2>&1; then
                export SELECTED_PROXY="$proxy"
                ok "成功匹配【真正可用】的加速代理: ${YELLOW}${proxy}${RESET}"
                return 0
            fi
            # 如果测到假代理，这里不会 return，会默默走到下一次循环，实现“真正的一个接一个试”
        fi
    done
    warn "所有 GitHub 代理均无法转发 API，将尝试使用直连盲跑..."
    export SELECTED_PROXY=""
}
