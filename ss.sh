...
EOF
    ...
    # 🛠️ 【一锤定音修复点】只有当至少存在一个本地容器服务时，才写入 depends_on 属性，杜绝悬空造成的语法破损
...
EOF
    ...
    # 🚀 【顺序优化】在启动前先切到工作目录，使用 down 进行环境强力洗牌，再拉起新配置
    ...
    # 🔗 【前置对齐】执行 artisan 之前，强行前置锚定当前工作路径
    cd "$BASE_DIR"
    ...
    # 🔄 【对齐修改点】管理员创完后，直接通过 down + up -d 的组合拳让刚刚生成的密钥和数据库连接全量对齐并生效
