#!/bin/bash
# ========================================
# Komari Traffic Hub 一键管理脚本 (含权限修复)
# ========================================
...
fix_permissions() {
    if [ -d "$APP_DIR/data" ]; then
        echo -e "${YELLOW}正在配置数据目录权限，防止 Permission denied 报错...${RESET}"
        # 赋予 777 权限，确保容器内非 root 用户 (如 UID 1000) 能正常读写锁文件
        chmod -R 777 "$APP_DIR/data"
        echo -e "${GREEN}权限配置完成！${RESET}"
    fi
}
...
menu() {
    while true; do
        ...
        echo -e "${GREEN}1) 安装启动 (含自动权限修复)${RESET}"
        echo -e "${GREEN}2) 更新服务${RESET}"
        echo -e "${GREEN}3) 重启服务 (并修复权限)${RESET}"
        echo -e "${GREEN}4) 查看日志${RESET}"
        echo -e "${GREEN}5) 查看状态${RESET}"
        echo -e "${GREEN}6) 修复权限 (单独执行)${RESET}"
        ...
        case $choice in
            ...
            4) view_logs ;;
            5) check_status ;;
            6) fix_permissions; read -p "按回车返回菜单..." ;;
            7) uninstall_app ;;
            0) exit 0 ;;
...
EOF
    ...
    # 2. 写入 docker-compose.yml 配置文件 (加入 user: "0:0" 彻底避免权限问题)
