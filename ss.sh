...
get_current_source_status() {
    case "$ID" in
        ...
        debian)
            ...
            if [ -f "$file" ]; then
                # 1. 抓取未注释的 URIs 或 deb 行
                local raw_line=$(grep -v '^#' "$file" | grep -E -i 'deb |URIs:' | head -n 1)
                
                if [ -n "$raw_line" ]; then
                    # 2. 如果检测到是 Debian 13 特有的本地 mirror 列表指针
                    if echo "$raw_line" | grep -q 'mirror+file://'; then
                        # 提取出本地 list 文件的实际路径（例如 /etc/apt/mirrors/debian.list）
                        local list_file=$(echo "$raw_line" | sed -E 's|.*mirror+file://||' | awk '{print $1}')
                        
                        # 顺藤摸瓜：如果这个 list 文件存在，进去抓第一行有效的 http/https 网址
                        if [ -f "$list_file" ] && grep -v '^#' "$list_file" | grep -q -E 'https?://'; then
                            local real_url=$(grep -v '^#' "$list_file" | grep -E 'https?://' | head -n 1)
                            local main_url=$(echo "$real_url" | sed -e 's|http://||' -e 's|https://||' -e 's|/.*||')
                            echo "$main_url (Debian官方镜像组)"
                        else
                            # 如果 list 文件是空的或者都是官方默认机制，给用户一个清晰的提示
                            echo "Debian 官方重定向源 (mirror+file)"
                        fi
                    else
                        # 3. 如果是普通的 http/https 源（比如用户切换过的阿里云、清华源等）
