...
install_remnawave() {
    ...
    if [[ "$redis_mode" == "2" ]]; then
        ...
        # 【修复】远程 Redis 必须封装为标准 REDIS_URL 连接串，否则后端无法识别
        if [[ -n "$rd_pass" ]]; then
            env_redis_cfg="REDIS_URL=\"redis://:${rd_pass}@${rd_host}:${rd_port}/${rd_db}\""
        else
            env_redis_cfg="REDIS_URL=\"redis://${rd_host}:${rd_port}/${rd_db}\""
        fi
    else
