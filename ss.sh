...
check_and_install() {
    ...
    case "$tool" in
        ...
        inetspeed)
            ...
            echo "inetspeed" | curl -fsSL https://raw.githubusercontent.com/tsosunchia/iNetSpeed-CLI/main/scripts/install.sh | bash || true
            ;;
...
run_inetspeed() {
    ...
    echo -e "${YELLOW}💡 提示: 已自动为你选择延迟最低的测速节点。${RESET}"
    echo "-------------------------------------"
    echo "" | inetspeed || true
    echo "-------------------------------------"
    read -p "测试完成，按回车返回面板..." dummy
...
while true; do
    ...
    echo -e " ${GREEN}6) 运行 Cloudflare 测速 (Rust-CLI)${RESET}"
    echo -e "${GREEN}--------------------------------${RESET}"
    echo -e " ${GREEN}0) 退出${RESET}"
    echo -e "${GREEN}================================${RESET}"
