...
download_hysteria() {
  ...
  for proxy in "${GITHUB_PROXY[@]}"; do
    ...
    info "正在通过代理 [${proxy:-直连}] 下载 Hysteria (路径1)..."

    if wget -q --show-progress --no-check-certificate -O "$_destination" "$_download_url"; then
      return 0
    fi

    _download_url="${proxy}${REPO_URL}/releases/download/$_version/hysteria-$OPERATING_SYSTEM-$ARCHITECTURE"
    info "正在通过代理 [${proxy:-直连}] 下载 Hysteria (路径2)..."
