...
# =========================================================
# Shadowsocks-Rust + Shadow-TLS 一体化管理脚本 (独立命名防冲突版)
# SS加密方式: 2022-blake3-aes-256-gcm
# 支持 IPv6 双栈，且服务名/路径均加 's' 防冲突
# =========================================================
...
print_node_info() {
    ...
    echo -e "${GREEN}====== Shadowsocks + Shadow-TLS (加s版) 配置 ======${RESET}"
...
execute_configuration_flow() {
    ...
    while true; do
        ...
        if [ "$is_modify_mode" = true ]; then
            ...
            printf "请输入SS内部自定义DNS (默认采纳系统: %s, 回--车直接采纳): " "$default_dns"
        fi
