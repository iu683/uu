#!/usr/bin/env bash
#
# Tuic 管理面板 (官方 v5 核心格式 & 404 自适应完美修复版)
...
get_current_port_display() {
  if [[ -f "$TUIC_CONFIG" ]]; then
    ...
    # 🌟 兼容性读取：优先尝试从符合 v5 标准的 server 字段中切出端口号，兜底读取旧 port 字段
...
write_and_show_config() {
  ...
  # 🌟 修复：完美对齐官方 v5 标准核心，使用符合规范的 server 顶层结构代替废弃的 port
...
tuic://$auth_uuid:$auth_pwd@$last_ip:$port?alpn=h3&congestion_control=bbr&sni=$tuic_domain&allow_insecure=1${hopping_param}#$HOSTNAME-tuic
