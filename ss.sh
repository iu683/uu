...
# =============================================================================
#  Xray VLESS-Reality 智能编号多实例管理面板 (稳定修复版)
# =============================================================================
...
menu_switch_instance() {
    ...
    # 使用 safe find 方式迭代，防止 set -e 闪退
    if [ -d "$CONFIG_DIR" ]; then
        local files
        files=$(find "$CONFIG_DIR" -maxdepth 1 -type f -name "config_*.json" 2>/dev/null)
        if [ -n "$files" ]; then
            for f in ${CONFIG_DIR}/config_*.json; do
                [ -e "$f" ] || continue
                ((count++))
                local name
                name=$(jq -r '._meta.alias // empty' "$f")
                [ -z "$name" ] && name=$(basename "$f" | sed 's/^config_//;s/\.json$//')
                instance_list+=("$name")
                
                local port_num
                port_num=$(jq -r '.inbounds[0].port // "未知"' "$f")
