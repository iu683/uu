...
# ===============================
# SSH 管理菜单脚本（支持 Alpine/Debian/CentOS）
# ===============================
...
setup_ssh_key() {
    ...
    if [ -f "$keypath" ]; then
        echo -e "${RED}❌ 文件已存在: $keypath${RESET}"
        echo "请手动删除或选择其他路径"
        return
    fi

    ssh-keygen -t ed25519 -N "" -q -f "$keypath"

    cat "${keypath}.pub" >> ~/.ssh/authorized_keys
    chmod 600 ~/.ssh/authorized_keys

    # 修改 sshd_config
    cp "$SSH_CONF" "${SSH_CONF}.bak.$(date +%s)"
...
disable_root_password() {
    ...
    echo -e "${GREEN}✅ root 密码登录已禁用（保留密钥登录）${RESET}"
}
