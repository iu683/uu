...
# ===============================
# 防火墙管理脚本（Alpine Linux 双栈 IPv4/IPv6 - Docker 完美兼容修正版）
# ===============================
...
save_and_enable_autoload() {
    ...
    if command -v rc-update >/dev/null 2>&1 && [ -f /etc/init.d/iptables ]; then
        ...
        echo -e "${GREEN}✅ 规则已保存并注册系统服务${RESET}"
    else
        ...
        echo -e "${GREEN}✅ 规则已成功保存到本地 (通过 local.d 脚本自启)${RESET}"
    fi
    read -p "按回车继续..."
}
...
ip_action() {
    ...
    case $action in
        ...
        drop)   
            ...
            # 【Docker 完美兼容】：黑名单同步拦截 Docker 容器网络流量
            if [ "$proto" = "iptables" ] && iptables -L DOCKER-USER -n >/dev/null 2>&1; then
...
ping_action() {
    local action=$1
    # 安全清理旧的 ICMP 规则
    while iptables -D INPUT -p icmp --icmp-type echo-request -j ACCEPT >/dev/null 2>&1; do :; done
    while iptables -D INPUT -p icmp -j DROP >/dev/null 2>&1; do :; done
    while ip6tables -D INPUT -p icmpv6 --icmpv6-type echo-request -j ACCEPT >/dev/null 2>&1; do :; done
    while ip6tables -D INPUT -p icmpv6 -j DROP >/dev/null 2>&1; do :; done
