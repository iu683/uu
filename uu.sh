...
# ================== 核心 Alpine 部署逻辑 (解决 404/解压失败 Bug) ==================
_download_and_install_binary() {
    ...
    # 方案 A：使用伪装的 Chrome 浏览器 User-Agent 尝试下载最新版
    local download_url_A="https://dl.nssurge.com/snell/snell-server-v${version}-linux-${sarch}.zip"
    # 方案 B：如果最新版被卡，备用无 'v' 前缀路径
    local download_url_B="https://dl.nssurge.com/snell/snell-server-${version}-linux-${sarch}.zip"
    # 方案 C：终极保底方案（目前已知绝对稳定存在的 v6 Beta2 镜像包）
    local download_url_C="https://dl.nssurge.com/snell/snell-server-v6.0.0b2-linux-${sarch}.zip"

    _info "正在通过智能路由下载 Snell v6 核心组件..."
    
    # 尝试 1：方案 A
    if curl -sL -A "Mozilla/5.0 (Windows NT 10.0; Win64; x64) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/120.0.0.0 Safari/537.36" -o "$tmp/snell.zip" --connect-timeout 15 "$download_url_A" && unzip -t "$tmp/snell.zip" >/dev/null 2>&1; then
        _info "方案 A (标准新版 v${version}) 下载并校验成功！"
    # 尝试 2：方案 B
    elif curl -sL -A "Mozilla/5.0 (Windows NT 10.0; Win64; x64) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/120.0.0.0 Safari/537.36" -o "$tmp/snell.zip" --connect-timeout 15 "$download_url_B" && unzip -t "$tmp/snell.zip" >/dev/null 2>&1; then
        _info "方案 B (变体路径) 下载并校验成功！"
    # 尝试 3：方案 C (终极兜底)
    else
        _warn "官方主动拦截或版本号未就绪，启动终极弹性回滚，下载 v6.0.0b2 保底包..."
        if ! curl -sL -A "Mozilla/5.0 (Windows NT 10.0; Win64; x64)" -o "$tmp/snell.zip" --connect-timeout 20 "$download_url_C" || ! unzip -t "$tmp/snell.zip" >/dev/null 2>&1; then
