...
get_status_text() {
    ...
    fq_check=$(sysctl -n net.core.default_qdisc 2>/dev/null || echo "未知")
    tfo_check=$(sysctl -n net.ipv4.tcp_fastopen 2>/dev/null || echo "0")
    ...
    # 2. 验证配置是否真正【注入内核并生效】（检查文件存在 且 关键参数符合脚本核心设定）
    if [ -f "$CONF_FILE" ] && [ "$fq_check" == "fq" ] && [ "$tfo_check" -eq 3 ]; then
