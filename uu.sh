#!/bin/bash
# =================================================================
# Vaultfleet 服务 Docker Compose 独立管理面板
# =================================================================
...
get_status_info() {
    ...
    if [ "$(docker ps -aq -f name=^/${CONTAINER_NAME}$)" ]; then
        ...
        [[ -z "$img_version" ]] && img_version="ghcr.io/momo-z/vaultfleet:latest"
        
        # 动态抓取映射到容器 8080 端口的宿主机实际端口
...
install_utils() {
    ...
    # 允许自定义宿主机端口
    echo -ne "${YELLOW}请输入 Vaultfleet 宿主机访问端口 [默认: 8080]: ${RESET}"
    read -r custom_port
...
EOF
    ...
    echo -e "${YELLOW}正在通过 Docker Compose 启动 Vaultfleet...${RESET}"
    ...
    echo -e "${GREEN}================================${RESET}"
    echo -e "${GREEN}      Vaultfleet 部署成功！      ${RESET}"
    echo -e "${GREEN}================================${RESET}"
...
update_utils() {
    ...
    echo -e "${YELLOW}正在从远端拉取 Vaultfleet 最新镜像...${RESET}"
    cd "$BASE_DIR" && docker compose pull
    docker compose up -d --remove-orphans
    echo -e "${GREEN}更新完成！Vaultfleet 已处于最新状态。${RESET}"
}

# 卸载 Vaultfleet
uninstall_utils() {
    echo -e "${RED}警告: 卸载如果清理数据，将永久丢失该服务内的全量历史数据！${RESET}"
    echo -ne "${YELLOW}确定要卸载并删除 Vaultfleet 容器吗？(y/n): ${RESET}"
    read -r confirm
    if [ "$confirm" = "y" ] || [ "$confirm" = "Y" ]; then
        if [ -f "$COMPOSE_FILE" ]; then
            ...
            if [ "$clean_data" = "y" ] || [ "$clean_data" = "Y" ]; then
                rm -rf "$BASE_DIR"
                echo -e "${GREEN}本地所有 Vaultfleet 历史数据已被彻底销毁。${RESET}"
            fi
        else
...
menu() {
    ...
    get_status_info
    echo -e "${GREEN}================================${RESET}"
    echo -e "${GREEN}  ◈  Vaultfleet 服务管理面板 ◈    ${RESET}"
    echo -e "${GREEN}================================${RESET}"
