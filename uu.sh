...
init_rules() {
    ...
    # 1. 强行创建 Alpine 官方服务目录
    mkdir -p /etc/iptables /etc/ip6tables
    
    # 2. 导出规则并确保包含 Alpine 服务必需的 COMMIT 标记
    iptables-save > /etc/iptables/rules-save 2>/dev/null || true
    if ! grep -q "COMMIT" /etc/iptables/rules-save; then echo "COMMIT" >> /etc/iptables/rules-save; fi

    ip6tables-save > /etc/ip6tables/rules-save 2>/dev/null || true
    if ! grep -q "COMMIT" /etc/ip6tables/rules-save; then echo "COMMIT" >> /etc/ip6tables/rules-save; fi
    
    # 3. 核心修复：将服务加入开机自启
    rc-update add iptables default >/dev/null 2>&1 || true
    rc-update add ip6tables default >/dev/null 2>&1 || true
    
    # 4. 核心修复：清理 Alpine 历史残留锁，强制启动并让服务真正 start 起来
    rc-service iptables zap >/dev/null 2>&1 || true
    rc-service ip6tables zap >/dev/null 2>&1 || true
