#!/usr/bin/env bash
#
# Sing-box (AnyTLS + Reality) Alpine 专属核心控制面板 (独立共存稳定版)
...
generate_or_use_key() {
  ...
  # 兼容带冒号和空格的解析机制
  PRIVATE_KEY=$(echo "$key_out" | grep -i "PrivateKey" | awk -F': ' '{print $2}' | tr -d '[:space:]')
  PUBLIC_KEY=$(echo "$key_out" | grep -i "PublicKey" | awk -F': ' '{print $2}' | tr -d '[:space:]')
  
  if [[ -z "$PRIVATE_KEY" || -z "$PUBLIC_KEY" ]]; then
    # 备用容错提取方案
    PRIVATE_KEY=$(echo "$key_out" | awk '/PrivateKey/ {print $2}' | tr -d '[:space:]')
    PUBLIC_KEY=$(echo "$key_out" | awk '/PublicKey/ {print $2}' | tr -d '[:space:]')
  fi
}
...
select_best_sni() {
    info "开始优选 SNI 延迟测试"
    local SNIS=(amd.com apps.mzstatic.com aws.com azure.microsoft.com bing.com www.apple.com www.microsoft.com)
    BEST_SNI=""
    BEST_TIME=999999

    for sni in "${SNIS[@]}"; do
        start=$(date +%s%N)
        timeout 3 openssl s_client -connect ${sni}:443 -servername ${sni} -brief </dev/null >/dev/null 2>&1
        if [ $? -eq 0 ]; then
            end=$(date +%s%N)
