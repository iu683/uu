...
apply_new_iptables() {
  ...
  if [[ -f "${CONFIG_DIR}/hopping.txt" ]]; then
    ...
    # 【核心修复】Alpine Linux 的 iptables nat 转发标准语法：使用减号表示范围
    if ! iptables -t nat -A PREROUTING -p udp --dport "${start_p}:${end_p}" -j REDIRECT --to-ports "$port" 2>/dev/null; then
       # 如果冒号失败，尝试标准减号语法（部分旧版本 Alpine 内核策略）
       iptables -t nat -A PREROUTING -p udp --dport "${start_p}-${end_p}" -j REDIRECT --to-ports "$port" 2>/dev/null || {
          # 终极保底：如果内核实在严苛，利用 iptables 符号范围扩展
          iptables -t nat -A PREROUTING -p udp -m mport --dports "${start_p}:${end_p}" -j REDIRECT --to-ports "$port" 2>/dev/null
       }
    fi
