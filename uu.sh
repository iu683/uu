...
# 获取 GitHub 官方最新正式版版本号
get_latest_version() {
  local latest_v
  latest_v=$(curl -sS https://api.github.com/repos/SagerNet/sing-box/releases/latest | jq -r .tag_name 2>/dev/null || true)
  # 去掉版本号前面的 "v"（如果是 "v1.13.12" 变成 "1.13.12"）
  latest_v=${latest_v#v}
  
  # 如果网络请求失败，给一个保底的稳定版本号
