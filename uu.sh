...
install_app() {
  ...
  # 检查 python3-venv 是否安装，如果缺少就自动安装
  if ! python3 -m venv --help >/dev/null 2>&1; then
    echo -e "${YELLOW}⚠️ python3-venv 未安装，正在自动安装...${RESET}"
    sudo apt update
    # 自动安装对应 python3-venv 包
    PYTHON_VER=$(python3 -c "import sys; print(f'python{sys.version_info.major}.{sys.version_info.minor}-venv')")
    sudo apt install -y $PYTHON_VER
    if [ $? -ne 0 ]; then
      echo -e "${RED}❌ 自动安装 python3-venv 失败，请手动安装${RESET}"
      read -p "按回车退出..."
      return
    fi
    echo -e "${GREEN}✅ python3-venv 安装完成${RESET}"
  fi
