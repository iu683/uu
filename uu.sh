...
install_app() {
    ...
    read -p "请输入 Hysteria 监听端口 [默认 443]: " input_port
    PORT=${input_port:-443}
    ...
    # 伪装网址
    read -p "是否启用伪装网址（masquerade）[y/N]: " enable_masq
    if [[ "$enable_masq" =~ ^[Yy]$ ]]; then
        read -p "请输入伪装网址（示例 https://bing.com）: " masq_input
        MASQ_URL=${masq_input:-https://bing.com}
    fi
