...
# 增强兼容版域名提取函数（彻底规避 sed 语法在不同系统下的转义报错）
get_all_domains() {
    [ ! -f "$CADDYFILE" ] && return
    # 1. 过滤出合法的块开头
    # 2. 剥离可能存在的 http:// 或 https://
    # 3. 提取第一列，并以冒号分界去除端口号
...
reload_caddy() {
    if systemctl is-active --quiet caddy; then
        if sudo caddy validate --config "$CADDYFILE" >/dev/null 2>&1; then
            ...
            echo -e "${RED}错误: Caddyfile 语法检查未通过，拒绝重载！请检查配置。${RESET}"
        fi
    else
    ...
    for DOMAIN in "${DOMAINS[@]}"; do
        ...
        if [ -n "$CERT_PATH" ] && [ -f "$CERT_PATH" ]; then
            ...
            if END_TS=$(date -d "$END_DATE" +%s 2>/dev/null); then
                ...
                # 如果 date 转换失败（精简系统环境），使用 openssl 自带机制兜底检查
...
view_sites() {
    ...
    if [ -n "$CERT_FILE" ] && [ -f "$CERT_FILE" ]; then
        ...
        echo -e "${YELLOW}${DOMAIN} - 本地未找到自动签发的证书（可能使用了自定义证书、或尚未签发成功）${RESET}"
    fi
    pause
}
