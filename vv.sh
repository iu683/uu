...
config_custom_api() {
    ...
    echo -e "${GREEN}================================${RESET}"
    echo -e "${GREEN}1. 快捷一键生成标准通用代理环境${RESET}"
    ...
    case $api_choice in
        1)
            ...
            if [ -n "$input_url" ] && [ -n "$input_key" ] && [ -n "$input_model" ] && [ -n "$input_submodel" ]; then
                # 【终极完美通用模板】
                # 1. 严格保留你需要的内置 env 参数与超时控制
                # 2. 补上最外层的 model: sonnet，欺骗客户端通过本地白名单校验，彻底解决闪退与死循环
                # 3. 补上 theme: dark 维持你的界面主题
...
EOF
                ...
                echo -e "\n${GREEN}✔ 成功！已按照修复版通用模板固化配置至: $SETTINGS_JSON${RESET}"
            else
