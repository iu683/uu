...
if [ -f "$ADMIN_SCRIPT" ]; then
    # 如果本地已经有固化文件，且当前不是在物理固定路径下运行的（比如用户又用了 curl 管道运行）
    if [ "$(readlink -f "$0" 2>/dev/null)" != "$ADMIN_SCRIPT" ]; then
        # 强行终止当前的临时内存流，直接交接给本地已安装的固定路径脚本执行
        exec "$ADMIN_SCRIPT" "$@"
    fi
else
    # 只有本地完全不存在 /usr/bin/snapshot.sh 时，才判定为“首次运行”，触发下载
    echo -e "${GREEN}检测到本地固定路径未安装，正在为您首次下载并固化到 ${ADMIN_SCRIPT}...${NC}"
    curl -sL https://raw.githubusercontent.com/iu683/uu/main/vv.sh > "$ADMIN_SCRIPT"
...
auto_copy_ssh_key() {
    ...
    echo -e "${GREEN}提示: 接下来需要输入一次远程服务器 [ $user@$ip ] 的 SSH 登录密码以完成密钥复制。${NC}"
