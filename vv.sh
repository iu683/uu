...
main() {
    ...
    while true; do
        ...
        if systemctl list-unit-files | grep -q "^smartdns\.service" && systemctl is-active --quiet smartdns; then
            smartdns_status_view="${GREEN}运行中${NC} ${YELLOW}(公网端口: 53)${NC}"
       ...
       # --- 精准版本对齐处理逻辑 ---
        ...
        echo -e "${GREEN} SNIProxy 版本:${NC}${YELLOW}${remote_sni_ver}${NC}"
        echo -e "${GREEN} SmartDNS 版本:${NC}${YELLOW}${remote_sdns_ver}${NC}"
