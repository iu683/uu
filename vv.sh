#!/bin/bash
# ==========================================
# ACME Pro 证书申请（本地管理：全面切换 ZeroSSL）
# ==========================================
...
get_system_status() {
    ...
    if [ -f "$acme_file" ]; then
        STATUS="${GREEN}运行中 (ZeroSSL)${RESET}"
...
standalone_issue(){
    ...
    # 切换为 zerossl server，并强制使用 ECC (ec-256)
...
ip_issue(){
    ...
    # ZeroSSL 原生支持 IP 签发，移除 shortlived 短周期标记，采用 90 天常规周期
    ...
    if [ $? -eq 0 ]; then
        ...
        red "证书申请失败。请确认："
        red "1. 该 IP 的 80 端口已在安全组/防火墙开放"
        red "2. 如果提示注册错误，请执行菜单 1 重新输入有效邮箱绑定 ZeroSSL"
    fi
    start_web
}
