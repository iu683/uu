...
backup() {
    ...
    if [[ ! -d "/etc/caady" ]]; then
        echo -e "${RED}未安装 Caady${RESET}"
        return
    fi

    TIMESTAMP=$(date +%F_%H-%M-%S)
    FILE="$DATA_DIR/caady_$TIMESTAMP.tar.gz"

    echo -e "${CYAN}开始备份 Caady 数据...${RESET}"

    tar czf "$FILE" \
        /etc/caady \
        /var/www/caady >> "$LOG_FILE" 2>&1

    if [[ $? -eq 0 ]]; then
        echo -e "${GREEN}备份成功${RESET}"
        send_tg "✅ Caady备份成功: $TIMESTAMP"
    else
...
restore() {
    ...
    systemctl stop caady 2>/dev/null

    tar xzf "$FILE" -C /

    systemctl start caady

    echo -e "${GREEN}恢复完成${RESET}"
    send_tg "🔄 Caady已恢复: $(basename "$FILE")"
}
