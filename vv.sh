...
install_nginx() {
    ...
    apk update
    if ! apk add nginx certbot curl bind-tools; then  # 去掉了不稳定的 certbot-nginx
        echo -e "${RED}安装失败，尝试自动修复...${RESET}"
        uninstall_nginx
        echo -e "${YELLOW}重新尝试安装...${RESET}"
        apk add nginx certbot curl bind-tools || {
    ...
    # === 🛡️ 校验防线：防止反代目标输入为空或有空格导致 Nginx 配置崩溃 ===
    while true; do
        echo -ne "${GREEN}请输入反代目标(例如:http://127.0.0.1:5788): ${RESET}"; read TARGET
        # 去除前后空格
        TARGET=$(echo "$TARGET" | sed 's/^[[:space:]]*//;s/[[:space:]]*$//')
        if [[ -z "$TARGET" ]]; then
            echo -e "${RED}❌ 错误: 反代目标不能为空，请重新输入！${RESET}"
        elif [[ "$TARGET" == *" "* ]]; then
            echo -e "${RED}❌ 错误: 反代目标中不能包含空格，请重新输入！${RESET}"
        elif [[ ! "$TARGET" =~ ^https?:// ]]; then
            echo -e "${RED}❌ 错误: 反代目标必须以 http:// 或 https:// 开头！${RESET}"
        else
            break
        fi
    done
    ...
    # ==================== 🛠️ 核心修改部分：改用 webroot 模式 ====================
    echo -e "${GREEN}正在通过 --webroot 模式向 Let's Encrypt 申请证书...${RESET}"
    
    # 创建通用临时验证目录，配合 Nginx 默认的 80 端口配置进行验证
    mkdir -p /var/www/localhost
    
    # 确保当前 Nginx 是存活的，webroot 必须依赖运行中的 Nginx 
    if ! pgrep nginx >/dev/null; then
        rc-service nginx zap >/dev/null 2>&1 || true
        rc-service nginx start >/dev/null 2>&1 || true
    fi

    # 使用 webroot 验证方式，不干扰、不重启 Nginx 进程
    certbot certonly --webroot \
        -w /var/www/localhost \
        -d "$DOMAIN" --non-interactive --agree-tos -m "$EMAIL" --register-unsafely-without-email
    ...
    if [ -f "/etc/letsencrypt/live/$DOMAIN/fullchain.pem" ]; then
        ...
        echo -e "${YELLOW}已终止反代配置写入，防止 Nginx 全盘瘫痪。请确保域名解析正确且 80 端口已被外部放行。${RESET}"
    fi

    # 自动续期定时任务 (适配 Alpine 的 rc-service 且改用更稳妥的 reload)
    if ! crontab -l 2>/dev/null | grep -q "certbot renew"; then
        (crontab -l 2>/dev/null; echo "0 2 * * * certbot renew --quiet --webroot -w /var/www/localhost --post-hook 'rc-service nginx reload'") | crontab -
    fi
    # ========================================================================
