...
get_status_text() {
    ...
    # 获取 TFO 值并强行过滤所有非数字字符，如果为空则默认为 0
    tfo_check=$(sysctl -n net.ipv4.tcp_fastopen 2>/dev/null | tr -cd '0-9')
    tfo_check=${tfo_check:-0}
    ...
    # 2. 验证配置是否真正【注入内核并生效】
    # 只要 FQ 队列对上了，且 TFO 只要开启了(值大于0)，并且配置文件确实存在，即认为应用成功
    if [ -f "$CONF_FILE" ] && [ "$fq_check" == "fq" ] && [ "$tfo_check" -gt 0 ]; then
