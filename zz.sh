...
menu() {
    while true; do
        clear
        echo -e "${GREEN}=== Apprise 管理菜单 ===${RESET}"
...
EOF
    ...
    echo
    echo -e "${GREEN}✅ Apprise 已启动${RESET}"
...
update_app() {
    ...
    docker compose pull
    docker compose up -d
    echo -e "${GREEN}✅ Apprise 更新完成${RESET}"
    read -p "按回车返回菜单..."
}
...
restart_app() {
    ...
    echo -e "${GREEN}✅ Apprise 已重启${RESET}"
    read -p "按回车返回菜单..."
}
...
uninstall_app() {
    ...
    docker compose down
    rm -rf "$APP_DIR"
    echo -e "${RED}✅ Apprise 已彻底卸载${RESET}"
    read -p "按回车返回菜单..."
}
