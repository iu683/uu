...
install_qbittorrent() {
    ...
    # 【智能防错核心】如果端口不是8080，提前把反跨站策略写进配置文件
    if [[ "$custom_port" != "8080" ]]; then
        echo -e "${YELLOW}检测到自定义端口 [${custom_port}]，正在自动为您跳过 HostHeader 验证防止 Unauthorized 错误...${RESET}"
        CONF_FILE="$CONF_DIR/qBittorrent.conf"
        # 如果文件不存在，先初始化一个基础格式
        [[ ! -f "$CONF_FILE" ]] && echo -e "[Preferences]" > "$CONF_FILE"
        
        # 检查是否已有配置，没有就追加
        if ! grep -q "WebUI\\\HostHeaderValidation" "$CONF_FILE"; then
            echo 'WebUI\HostHeaderValidation=false' >> "$CONF_FILE"
        else
            # 如果已经有了但被设为了 true，用 sed 改成 false
