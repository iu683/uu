...
install_caddy() {
    ...
    sudo chmod 644 /etc/caddy/Caddyfile || sudo touch /etc/caddy/Caddyfile

    # 重新加载 systemd 并启用服务
    sudo systemctl daemon-reload
    sudo systemctl enable caddy

    # 验证配置，如果不存在 Caddyfile 或空文件，先创建空文件
    if ! sudo caddy validate --config /etc/caddy/Caddyfile >/dev/null 2>&1; then
        echo -e "${YELLOW}Caddyfile 不存在或语法错误，创建空文件${RESET}"
        echo -e "" | sudo tee /etc/caddy/Caddyfile >/dev/null
    fi

    sudo systemctl restart caddy
    if systemctl is-active --quiet caddy; then
        echo -e "${GREEN}Caddy 已启动并可用${RESET}"
    else
        echo -e "${RED}Caddy 启动失败，请检查 /etc/caddy/Caddyfile 语法${RESET}"
