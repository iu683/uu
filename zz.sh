...
tools=(
    ...
    [8]="tar:压缩解压:help"
    [9]="tmux:多路后台:run"
    [10]="ffmpeg:视频编码直播推流:help"
    [11]="btop:监控:run"
    [12]="ranger:文件管理:run_root"
    [13]="ncdu:磁盘占用查看:run_root"
    [14]="fzf:全局搜索:run_root"
    [15]="vim:文本编辑器:help_root"
    [16]="nano:文本编辑器:help_root"
    [17]="git:版本控制:help_root"
    [20]="iperf3:网络工具:sys"
    [21]="mtr:网络工具:sys"
)
...
show_menu() {
    ...
    for i in $(printf "%s\n" "${!tools[@]}" | sort -n); do
        IFS=":" read -r cmd desc mode <<< "${tools[$i]}"
        ...
        printf "${GREEN}[%02d]${RESET} %-12s %-22s %b\n" \
            "$i" "$cmd" "$desc" "$status"
    done
...
while true; do
    ...
    if [[ -n "${tools[$sub_choice]}" ]]; then
        IFS=":" read -r cmd desc mode <<< "${tools[$sub_choice]}"
        tool="$cmd"
        clear
        case "$mode" in
