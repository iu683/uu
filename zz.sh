...
if [ -n "$REAL_CMD" ] || [ -d "/opt/1panel" ]; then
    ...
    if [ -n "$REAL_CMD" ]; then
        ...
        port=$(echo "$user_info" | grep -i "port" | awk -F': ' '{print $2}' | tr -d ' \r\n ')
        entrance=$(echo "$user_info" | grep -i "entrance" | awk -F': ' '{print $2}' | tr -d ' \r\n ')
    fi
    ...
    # 如果还是空，通过系统监听端口精准反查（原生模式优化）
    if [ -z "$port" ]; then
        port=$(ss -tulnp 2>/dev/null | grep -E "1panel" | awk '{print $4}' | awk -F':' '{print $NF}' | sort -u | head -n 1 | tr -d ' \r\n ')
    fi
    
    # 最终输出，如果没有读取到才显示未侦测
    echo -e "${GREEN}面板端口:${RESET} ${YELLOW}${port:-未检测到新端口}${RESET}"
else
