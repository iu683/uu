#!/bin/bash
# =================================================================
# 呆呆面板 (daidai-panel) 定时任务服务 Docker Compose 独立管理面板
# =================================================================
...
get_status_info() {
    ...
    if [ "$(docker ps -aq -f name=^/${CONTAINER_NAME}$)" ]; then
        ...
        [[ -z "$img_version" ]] && img_version="linzixuanzz/daidai-panel:latest"
...
install_utils() {
    ...
    echo -e "${YELLOW}提示: 直接回车将默认采用同级路径下的 Dumb-Panel 文件夹。${RESET}"
    
    echo -ne "${YELLOW}请输入数据挂载路径 [默认: ./Dumb-Panel]: ${RESET}"
    read -r input_data
    local path_data_raw="${input_data:-./Dumb-Panel}"
    local real_path_data=$(get_real_path "$path_data_raw" "./Dumb-Panel")
    ...
    # 允许自定义宿主机端口
    echo -ne "${YELLOW}请输入呆呆面板宿主机外部访问端口 [默认: 5700]: ${RESET}"
    read -r custom_port
...
EOF
    ...
    echo -e "${YELLOW}正在通过 Docker Compose 启动 呆呆面板...${RESET}"
...
update_utils() {
    ...
    echo -e "${YELLOW}正在从远端拉取 呆呆面板 最新镜像...${RESET}"
...
# 卸载 daidai-panel
uninstall_utils() {
    echo -e "${RED}警告: 卸载如果清理数据，将永久丢失您的定时任务、脚本及所有面板配置！${RESET}"
    echo -ne "${YELLOW}确定要卸载并删除 呆呆面板 容器吗？(y/n): ${RESET}"
    read -r confirm
    if [ "$confirm" = "y" ] || [ "$confirm" = "Y" ]; then
        if [ -f "$COMPOSE_FILE" ]; then
            cd "$BASE_DIR" && docker compose down
            echo -e "${GREEN}容器已停止并移除。${RESET}"
            echo -ne "${RED}【高风险】是否同时彻底删除本地全量挂载的 Dumb-Panel 数据文件夹？(y/n): ${RESET}"
            read -r clean_data
            if [ "$clean_data" = "y" ] || [ "$clean_data" = "Y" ]; then
                rm -rf "$BASE_DIR"
                echo -e "${GREEN}本地所有 呆呆面板 历史数据已被彻底销毁。${RESET}"
            fi
        else
