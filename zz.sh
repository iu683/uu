...
install_tun2socks() {
    ...
    # 尝试直接通过常规网络获取
    DOWNLOAD_URL=$(curl -s --connect-timeout 5 https://api.github.com/repos/$REPO/releases/latest | grep "browser_download_url" | grep "linux-x86_64" | cut -d '"' -f 4 || true)
    
    # 如果常规网络失败（纯 v6 环境无法直接请求 GitHub v4 地址），切换临时修改 DNS 的方法
    if [ -z "$DOWNLOAD_URL" ]; then
        warning "常规连接失败，检测到可能处于纯 IPv6 环境，正在通过临时覆写 DNS 尝试轮询..."
        
        # 备份原有的 resolv.conf 并在退出函数时确保恢复它
        local dns_backup=$(cat /etc/resolv.conf)
        trap 'echo "$dns_backup" > /etc/resolv.conf' RETURN

        for dns64 in "${ALTERNATE_DNS64_SERVERS[@]}"; do
            info "正在切换临时 DNS 到 DNS64 [$dns64]..."
            echo "nameserver $dns64" > /etc/resolv.conf
            
            DOWNLOAD_URL=$(curl -s --connect-timeout 5 https://api.github.com/repos/$REPO/releases/latest | grep "browser_download_url" | grep "linux-x86_64" | cut -d '"' -f 4 || true)
            if [ -n "$DOWNLOAD_URL" ]; then
                success "成功通过 DNS64 服务器 [$dns64] 解析到下载链接。"
                break
            fi
        done
    fi
    ...
    if ! curl -L --connect-timeout 10 -o "$BINARY_PATH" "$DOWNLOAD_URL"; then
        error "下载核心文件失败，请检查网络。"
        return 1
    fi
    chmod +x "$BINARY_PATH"

    # 如果之前修改了 DNS，此处函数退出后会通过上方定义的 trap 自动还原为原本的 resolv.conf
