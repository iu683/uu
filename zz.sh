...
install_utils() {
    ...
    script -q -c "docker run -i -t --rm ghcr.io/steveiliop56/tinyauth:latest user create --interactive" "$tmp_log" > /dev/null

    local extracted_user=$(sed -r "s/\x1B\[([0-9]{1,3}(;[0-9]{1,3})*)?[mGK]//g" "$tmp_log" | grep -a "User created user=" | awk -F'user=' '{print $2}' | tr -d '\r' | tr -d '\n')
    rm -f "$tmp_log"
