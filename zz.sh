...
# ================= 核心重构：远程流式执行自我克隆与快捷键绑定 =================
init_and_clone_self() {
    mkdir -p "$FRP_INSTALL_DIR"

    # 1. 严格过滤远程管道路径判定
    if [[ "$0" != "$LOCAL_SCRIPT" ]]; then
        echo -e "${YELLOW}[系统初始化] 检测到远程流式运行，正在下载固化到本地...${RESET}"
        
        # 2. 避免消耗管道，采用干净的直接网络下载覆盖
        if ! curl -sL https://raw.githubusercontent.com/iu683/uu/main/zz.sh > "$LOCAL_SCRIPT"; then
            echo -e "${RED}[核心错误] 脚本下载失败，请检查网络链接或目标库文件路径！${RESET}"
            exit 1
        fi
        ...
        if [ -w /usr/bin ]; then
            ...
            echo -e "${GREEN}[快捷键] 已创建全局命令 'p'，下次输入 p 即可快速打开面板。${RESET}"
        else
            ...
            echo -e "${GREEN}[快捷键] 已向环境配置文件注入 alias p 快捷键。${RESET}"
        fi
        
        # 4. 关键：阻断当前已被部分消耗的流，直接将权限平移交由本地实体文件控制
        exec "$LOCAL_SCRIPT" "$@"
    fi
}
...
# 第一步：激活固化克隆校验（若为远程，此步将 exec 移交且阻断当前进程往下走）
init_and_clone_self
