...
[ -z "$cpu_model" ] && cpu_model=$(lscpu 2>/dev/null | grep "Model name" | cut -d: -f2)
    ...
    # 清理不需要的部分：
    # - 删除 "CPU @ xxxGHz"
    # - 删除 "xx-Core Processor"
    # - 删除多余的 "CPU" 或 "Processor"
cpu_model=$(echo "$cpu_model" | sed -E 's/CPU @ .*//; s/[0-9]+-Core Processor//; s/CPU//g; s/Processor//g' | xargs)   
...
SUB_MENU[9]="87 脚本短链|89 网站部署|90 OpenClaw|91 Emby反代|92 GProxy加速|93 Akile优先DNS|94 自动机场签到|95 1panelapps管理|96 关闭V1SSH|97 卸载哪吒Agent|98 卸载komariAgent"
...
execute_choice() {
    case "$1" in
		...
		90) bash <(curl -sL https://raw.githubusercontent.com/sistarry/toolbox/main/Docker/OpenClaw.sh) ;;
